  ...
  (
    ...
    React.useEffect(() => {
      ...
      const handlePointerDown = (event: PointerEvent) => {
        ...
        const targetInsideSidebarControl = Boolean(event.target.closest("[data-sidebar-control='true']"))

        if (shouldCollapseSidebarFromOutsidePointer({
          renderMode,
          state,
          targetInsideSidebar,
          targetInsideSidebarPortal: targetInsideSidebarPortal || targetInsideSidebarControl,
        })) {
          setOpen(false)
        }
      }
    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar="sidebar"
            className="flex h-full w-full flex-col bg-sidebar shadow-[inset_0_1px_0_hsl(var(--sidebar-border)/0.42)] group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
          >
            {children}
          </div>
        </div>
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex flex-col gap-2 p-2.5 group-data-[collapsible=icon]:items-center", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="footer"
      className={cn("mt-auto flex shrink-0 flex-col gap-2 p-2.5 group-data-[collapsible=icon]:items-center", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden group-data-[collapsible=icon]:items-center group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-semibold text-sidebar-foreground/75 outline-none ring-sidebar-ring transition-[margin,opacity,color,background-color] ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "after:absolute after:-inset-y-2 after:-inset-x-1 after:md:hidden",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full max-w-full min-w-0 items-center gap-2 overflow-hidden rounded-lg p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding,background-color,color] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-primary data-[active=true]:font-medium data-[active=true]:text-primary-foreground data-[active=true]:shadow-[0_0_24px_hsl(var(--brand-orange-glow)/0.12)] data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-9 group-data-[collapsible=icon]:!justify-center group-data-[collapsible=icon]:!p-0 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs group-data-[collapsible=icon]:!size-9",
...
>(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-1 top-1.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-[opacity,transform,background-color,color] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 peer-hover/menu-button:text-sidebar-accent-foreground group-data-[side=right]:left-1 group-data-[side=right]:right-auto [&>svg]:size-4 [&>svg]:shrink-0",
        // Increases the hit area of the button on mobile.
        "after:absolute after:-inset-y-2 after:-inset-x-1 after:md:hidden",
...
>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      ...
      "peer-data-[size=default]/menu-button:top-1.5",
      "peer-data-[size=lg]/menu-button:top-2.5",
      "group-data-[collapsible=icon]:right-1.5 group-data-[collapsible=icon]:top-1.5 group-data-[collapsible=icon]:size-1.5 group-data-[collapsible=icon]:min-w-0 group-data-[collapsible=icon]:rounded-full group-data-[collapsible=icon]:bg-primary group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:text-transparent group-data-[collapsible=icon]:shadow-[0_0_0_2px_hsl(var(--sidebar-background))]",
      className
    )}
    {...props}
  />
