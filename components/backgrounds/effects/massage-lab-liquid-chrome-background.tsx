...
import type { BackgroundEffectProps, MassageLabLiquidChromeOptions } from "./css-backgrounds"

type ResolvedLiquidChromeOptions = Required<MassageLabLiquidChromeOptions>
...
export default function MassageLabLiquidChromeBackground({
  ...
  const options = useMemo(
    () => resolveLiquidChromeOptions(massageLabLiquidChrome),
    [massageLabLiquidChrome],
  )
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.massageLabLiquidChromeCanvas, className)}
    />
  )
}
...
function resolveLiquidChromeOptions(options?: MassageLabLiquidChromeOptions): ResolvedLiquidChromeOptions {
  return {
    ...
    interactive: options?.interactive ?? DEFAULT_MASSAGELAB_LIQUID_CHROME.interactive,
  }
}
