...
import type { BackgroundEffectProps, MassageLabLetterGlitchOptions } from "./css-backgrounds"
...
type ResolvedLetterGlitchOptions = Required<MassageLabLetterGlitchOptions>

const DEFAULT_CHARACTERS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$&*()-_+=/[]{};:<>.,0123456789"
const DEFAULT_MASSAGELAB_LETTER_GLITCH: ResolvedLetterGlitchOptions = {
  colorOne: "#2B4539",
  colorTwo: "#61DCA3",
  colorThree: "#61B3DC",
  glitchSpeed: 50,
...
export default function MassageLabLetterGlitchBackground({
  ...
  const options = useMemo(
    () => resolveLetterGlitchOptions(massageLabLetterGlitch),
    [massageLabLetterGlitch],
  )
  ...
  return (
    <div className={cn(styles.massageLabLetterGlitch, className)} aria-hidden="true">
      <canvas ref={canvasRef} className={styles.massageLabLetterGlitchCanvas} />
      {options.outerVignette ? <span className={styles.massageLabLetterGlitchOuterVignette} /> : null}
      {options.centerVignette ? <span className={styles.massageLabLetterGlitchCenterVignette} /> : null}
    </div>
  )
}

function resolveLetterGlitchOptions(options?: MassageLabLetterGlitchOptions): ResolvedLetterGlitchOptions {
  return {
    colorOne: resolveHex(options?.colorOne, DEFAULT_MASSAGELAB_LETTER_GLITCH.colorOne),
    colorTwo: resolveHex(options?.colorTwo, DEFAULT_MASSAGELAB_LETTER_GLITCH.colorTwo),
    colorThree: resolveHex(options?.colorThree, DEFAULT_MASSAGELAB_LETTER_GLITCH.colorThree),
    glitchSpeed: resolveNumber(options?.glitchSpeed, DEFAULT_MASSAGELAB_LETTER_GLITCH.glitchSpeed, 16, 500),
    centerVignette: options?.centerVignette ?? DEFAULT_MASSAGELAB_LETTER_GLITCH.centerVignette,
    outerVignette: options?.outerVignette ?? DEFAULT_MASSAGELAB_LETTER_GLITCH.outerVignette,
    smooth: options?.smooth ?? DEFAULT_MASSAGELAB_LETTER_GLITCH.smooth,
    characters: typeof options?.characters === "string" && options.characters.trim().length > 0
      ? options.characters.slice(0, 120)
      : DEFAULT_MASSAGELAB_LETTER_GLITCH.characters,
  }
}
