...
import type { BackgroundEffectProps, MassageLabDarkVeilOptions } from "./css-backgrounds"

type ResolvedDarkVeilOptions = Required<MassageLabDarkVeilOptions>

type DarkVeilResources = {
  ...
  uniforms: {
    ...
    scan: WebGLUniformLocation
    scanFrequency: WebGLUniformLocation
    warp: WebGLUniformLocation
  }
}

const DEFAULT_MASSAGELAB_DARK_VEIL: ResolvedDarkVeilOptions = {
  hueShift: 0,
  noiseIntensity: 0,
  scanlineIntensity: 0,
  speed: 0.5,
...
export default function MassageLabDarkVeilBackground({
  ...
  const hueShift = massageLabDarkVeil?.hueShift
  const noiseIntensity = massageLabDarkVeil?.noiseIntensity
  const scanlineIntensity = massageLabDarkVeil?.scanlineIntensity
  const speed = massageLabDarkVeil?.speed
  const scanlineFrequency = massageLabDarkVeil?.scanlineFrequency
  const warpAmount = massageLabDarkVeil?.warpAmount
  const resolutionScale = massageLabDarkVeil?.resolutionScale
  const options = useMemo(
    () => resolveDarkVeilOptions({
      hueShift,
      noiseIntensity,
      scanlineIntensity,
      speed,
  ...
  useEffect(() => {
    ...
    const resources = createDarkVeilResources(context)
    ...
    const bindStaticUniforms = () => {
      ...
      context.uniform1f(resources.uniforms.noise, options.noiseIntensity)
      context.uniform1f(resources.uniforms.scan, options.scanlineIntensity)
      context.uniform1f(resources.uniforms.scanFrequency, options.scanlineFrequency)
      context.uniform1f(resources.uniforms.warp, options.warpAmount)
    }
    ...
    const resizeCanvas = () => {
      ...
      canvas.width = Math.max(1, Math.floor(width * dpr * options.resolutionScale))
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabDarkVeil, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabDarkVeilCanvas} />
    </div>
  )
}

function createDarkVeilResources(context: WebGLRenderingContext): DarkVeilResources | null {
  ...
  return {
    ...
    uniforms: {
      ...
      scan: getUniformLocation(context, program, "uScan"),
      scanFrequency: getUniformLocation(context, program, "uScanFreq"),
      warp: getUniformLocation(context, program, "uWarp"),
    },
  }
}
...
function resolveDarkVeilOptions(options: MassageLabDarkVeilOptions | undefined): ResolvedDarkVeilOptions {
  return {
    hueShift: clampNumber(options?.hueShift, DEFAULT_MASSAGELAB_DARK_VEIL.hueShift, -180, 180),
    noiseIntensity: clampNumber(options?.noiseIntensity, DEFAULT_MASSAGELAB_DARK_VEIL.noiseIntensity, 0, 1),
    scanlineIntensity: clampNumber(options?.scanlineIntensity, DEFAULT_MASSAGELAB_DARK_VEIL.scanlineIntensity, 0, 1),
    speed: clampNumber(options?.speed, DEFAULT_MASSAGELAB_DARK_VEIL.speed, 0, 2),
    scanlineFrequency: clampNumber(options?.scanlineFrequency, DEFAULT_MASSAGELAB_DARK_VEIL.scanlineFrequency, 0, 40),
    warpAmount: clampNumber(options?.warpAmount, DEFAULT_MASSAGELAB_DARK_VEIL.warpAmount, 0, 2),
    resolutionScale: clampNumber(options?.resolutionScale, DEFAULT_MASSAGELAB_DARK_VEIL.resolutionScale, 0.25, 1),
  }
}
