...
import type { BackgroundEffectProps, MassageLabPlasmaWaveOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedPlasmaWaveOptions = Omit<Required<MassageLabPlasmaWaveOptions>, "colors"> & {
...
const DEFAULT_MASSAGELAB_PLASMA_WAVE: ResolvedPlasmaWaveOptions = {
  xOffset: 0,
  yOffset: 0,
  rotationDeg: 0,
  focalLength: 0.8,
...
export default function MassageLabPlasmaWaveBackground({
  ...
  const options = useMemo(
    () => resolvePlasmaWaveOptions(massageLabPlasmaWave),
    [massageLabPlasmaWave],
  )
  ...
  return (
    <div className={cn(styles.effectLayer, styles.massageLabPlasmaWave, className)} ref={containerRef}>
      <canvas className={styles.massageLabPlasmaWaveCanvas} ref={canvasRef} />
    </div>
  )
}
...
function resolvePlasmaWaveOptions(options?: MassageLabPlasmaWaveOptions): ResolvedPlasmaWaveOptions {
  return {
    xOffset: clampNumber(options?.xOffset, DEFAULT_MASSAGELAB_PLASMA_WAVE.xOffset, -800, 800),
    yOffset: clampNumber(options?.yOffset, DEFAULT_MASSAGELAB_PLASMA_WAVE.yOffset, -800, 800),
    rotationDeg: clampNumber(options?.rotationDeg, DEFAULT_MASSAGELAB_PLASMA_WAVE.rotationDeg, -180, 180),
    focalLength: clampNumber(options?.focalLength, DEFAULT_MASSAGELAB_PLASMA_WAVE.focalLength, 0.2, 2),
    speed1: clampNumber(options?.speed1, DEFAULT_MASSAGELAB_PLASMA_WAVE.speed1, 0, 0.5),
    speed2: clampNumber(options?.speed2, DEFAULT_MASSAGELAB_PLASMA_WAVE.speed2, 0, 0.5),
    dir2: options?.dir2 === -1 ? -1 : 1,
    bend1: clampNumber(options?.bend1, DEFAULT_MASSAGELAB_PLASMA_WAVE.bend1, 0, 3),
    bend2: clampNumber(options?.bend2, DEFAULT_MASSAGELAB_PLASMA_WAVE.bend2, 0, 3),
    colors: normalizeColorList(options?.colors, DEFAULT_MASSAGELAB_PLASMA_WAVE.colors),
  }
}
