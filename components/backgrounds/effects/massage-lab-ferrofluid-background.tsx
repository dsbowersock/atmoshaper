...
import type { BackgroundEffectProps, MassageLabFerrofluidOptions } from "./css-backgrounds"

type FlowDirection = NonNullable<MassageLabFerrofluidOptions["flowDirection"]>
type RgbColor = [number, number, number]
type ResolvedFerrofluidOptions = Required<Omit<MassageLabFerrofluidOptions, "colors">> & {
  colors: string[]
}
...
const DEFAULT_MASSAGELAB_FERROFLUID: ResolvedFerrofluidOptions = {
...
export default function MassageLabFerrofluidBackground({
  ...
  const colorsKey = (massageLabFerrofluid?.colors ?? []).join("|")
  const speed = massageLabFerrofluid?.speed
  const scale = massageLabFerrofluid?.scale
  const turbulence = massageLabFerrofluid?.turbulence
  const fluidity = massageLabFerrofluid?.fluidity
  const rimWidth = massageLabFerrofluid?.rimWidth
  const sharpness = massageLabFerrofluid?.sharpness
  const shimmer = massageLabFerrofluid?.shimmer
  const glow = massageLabFerrofluid?.glow
  const flowDirection = massageLabFerrofluid?.flowDirection
  const opacity = massageLabFerrofluid?.opacity
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabFerrofluid, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabFerrofluidCanvas} />
    </div>
  )
}
...
function resolveFerrofluidOptions(options: MassageLabFerrofluidOptions | undefined): ResolvedFerrofluidOptions {
  return {
    colors: resolveFerrofluidColors(options?.colors),
    speed: clampNumber(options?.speed, DEFAULT_MASSAGELAB_FERROFLUID.speed, 0.05, 2),
    scale: clampNumber(options?.scale, DEFAULT_MASSAGELAB_FERROFLUID.scale, 0.5, 4),
    turbulence: clampNumber(options?.turbulence, DEFAULT_MASSAGELAB_FERROFLUID.turbulence, 0, 2),
    fluidity: clampNumber(options?.fluidity, DEFAULT_MASSAGELAB_FERROFLUID.fluidity, 0.001, 0.4),
    rimWidth: clampNumber(options?.rimWidth, DEFAULT_MASSAGELAB_FERROFLUID.rimWidth, 0.03, 0.5),
    sharpness: clampNumber(options?.sharpness, DEFAULT_MASSAGELAB_FERROFLUID.sharpness, 0.5, 6),
    shimmer: clampNumber(options?.shimmer, DEFAULT_MASSAGELAB_FERROFLUID.shimmer, 0, 4),
    glow: clampNumber(options?.glow, DEFAULT_MASSAGELAB_FERROFLUID.glow, 0.1, 5),
    flowDirection: resolveFlowDirection(options?.flowDirection),
    opacity: clampNumber(options?.opacity, DEFAULT_MASSAGELAB_FERROFLUID.opacity, 0.05, 1),
  }
}
...
function resolveFerrofluidColors(colors: string[] | undefined) {
  ...
  return sanitized.length ? sanitized : DEFAULT_MASSAGELAB_FERROFLUID.colors
}
...
function prepFerrofluidColors(colors: string[]) {
  ...
  for (let index = 0; index < MAX_FERROFLUID_COLORS; index += 1) {
    arr.push(parseHexColorToRgb(base[Math.min(index, base.length - 1)] ?? DEFAULT_MASSAGELAB_FERROFLUID.colors[0]))
  }
...
function resolveFlowDirection(value: FlowDirection | undefined): FlowDirection {
  return value === "up" || value === "left" || value === "right" || value === "down"
    ? value
    : DEFAULT_MASSAGELAB_FERROFLUID.flowDirection
}
