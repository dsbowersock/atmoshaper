...
import type { BackgroundEffectProps, MassageLabSilkOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedSilkOptions = Required<MassageLabSilkOptions>
...
const DEFAULT_MASSAGELAB_SILK: ResolvedSilkOptions = {
  color: "#7B7481",
  speed: 5,
  scale: 1,
  noiseIntensity: 1.5,
...
export default function MassageLabSilkBackground({
  ...
  const color = massageLabSilk?.color
  const speed = massageLabSilk?.speed
  const scale = massageLabSilk?.scale
  const noiseIntensity = massageLabSilk?.noiseIntensity
  const rotation = massageLabSilk?.rotation
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabSilk, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabSilkCanvas} />
    </div>
  )
}
...
function resolveSilkOptions(options: MassageLabSilkOptions | undefined): ResolvedSilkOptions {
  return {
    color: resolveHexColor(options?.color, DEFAULT_MASSAGELAB_SILK.color),
    speed: resolveNumber(options?.speed, DEFAULT_MASSAGELAB_SILK.speed, 0, 10),
    scale: resolveNumber(options?.scale, DEFAULT_MASSAGELAB_SILK.scale, 0.2, 4),
    noiseIntensity: resolveNumber(
      options?.noiseIntensity,
      DEFAULT_MASSAGELAB_SILK.noiseIntensity,
      0,
      4,
    ),
    rotation: resolveNumber(options?.rotation, DEFAULT_MASSAGELAB_SILK.rotation, -Math.PI, Math.PI),
  }
}

function parseHexColorToRgb(color: string): RgbColor {
  const normalized = resolveHexColor(color, DEFAULT_MASSAGELAB_SILK.color).replace("#", "")
  return [
