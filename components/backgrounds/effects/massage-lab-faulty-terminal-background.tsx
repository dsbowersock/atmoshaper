...
import type { BackgroundEffectProps, MassageLabFaultyTerminalOptions } from "./css-backgrounds"

type ResolvedFaultyTerminalOptions = Required<MassageLabFaultyTerminalOptions>
...
const DEFAULT_MASSAGELAB_FAULTY_TERMINAL: ResolvedFaultyTerminalOptions = {
  scale: 1,
  gridMulX: 2,
  gridMulY: 1,
  digitSize: 1.5,
...
export default function MassageLabFaultyTerminalBackground({
  ...
  const options = useMemo(
    () => resolveFaultyTerminalOptions(massageLabFaultyTerminal),
    [massageLabFaultyTerminal],
  )
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.massageLabFaultyTerminalCanvas, className)}
    />
  )
}
...
function resolveFaultyTerminalOptions(
  options: MassageLabFaultyTerminalOptions | undefined,
): ResolvedFaultyTerminalOptions {
  return {
    scale: resolveNumber(options?.scale, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.scale, 0.25, 4),
    gridMulX: resolveNumber(options?.gridMulX, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.gridMulX, 0.25, 6),
    gridMulY: resolveNumber(options?.gridMulY, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.gridMulY, 0.25, 6),
    digitSize: resolveNumber(options?.digitSize, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.digitSize, 0.5, 4),
    timeScale: resolveNumber(options?.timeScale, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.timeScale, 0, 2),
    scanlineIntensity: resolveNumber(
      options?.scanlineIntensity,
      DEFAULT_MASSAGELAB_FAULTY_TERMINAL.scanlineIntensity,
      0,
      2,
    ),
    glitchAmount: resolveNumber(options?.glitchAmount, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.glitchAmount, 0, 3),
    flickerAmount: resolveNumber(options?.flickerAmount, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.flickerAmount, 0, 2),
    noiseAmp: resolveNumber(options?.noiseAmp, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.noiseAmp, 0, 2),
    chromaticAberration: resolveNumber(
      options?.chromaticAberration,
      DEFAULT_MASSAGELAB_FAULTY_TERMINAL.chromaticAberration,
      0,
      8,
    ),
    dither: resolveNumber(options?.dither, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.dither, 0, 255),
    curvature: resolveNumber(options?.curvature, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.curvature, 0, 1),
    tint: resolveHex(options?.tint, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.tint),
    mouseReact: options?.mouseReact ?? DEFAULT_MASSAGELAB_FAULTY_TERMINAL.mouseReact,
    mouseStrength: resolveNumber(options?.mouseStrength, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.mouseStrength, 0, 2),
    pageLoadAnimation: options?.pageLoadAnimation ?? DEFAULT_MASSAGELAB_FAULTY_TERMINAL.pageLoadAnimation,
    brightness: resolveNumber(options?.brightness, DEFAULT_MASSAGELAB_FAULTY_TERMINAL.brightness, 0.1, 3),
  }
}
