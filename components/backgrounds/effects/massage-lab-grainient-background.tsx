...
import type { BackgroundEffectProps, MassageLabGrainientOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedGrainientOptions = Required<MassageLabGrainientOptions>
...
const DEFAULT_MASSAGELAB_GRAINIENT: ResolvedGrainientOptions = {
  timeSpeed: 0.25,
  colorBalance: 0,
  warpStrength: 1,
  warpFrequency: 5,
...
export default function MassageLabGrainientBackground({
  className,
  massageLabGrainient,
}: BackgroundEffectProps) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const options = useMemo(
    () => resolveGrainientOptions(massageLabGrainient),
    [massageLabGrainient],
  )
  ...
  return (
    <div className={cn(styles.massageLabGrainient, className)} aria-hidden="true">
      <canvas ref={canvasRef} className={styles.massageLabGrainientCanvas} />
    </div>
  )
}
...
function resolveGrainientOptions(options?: MassageLabGrainientOptions): ResolvedGrainientOptions {
  return {
    timeSpeed: clampNumber(options?.timeSpeed, DEFAULT_MASSAGELAB_GRAINIENT.timeSpeed, 0, 2),
    colorBalance: clampNumber(options?.colorBalance, DEFAULT_MASSAGELAB_GRAINIENT.colorBalance, -1, 1),
    warpStrength: clampNumber(options?.warpStrength, DEFAULT_MASSAGELAB_GRAINIENT.warpStrength, 0, 5),
    warpFrequency: clampNumber(options?.warpFrequency, DEFAULT_MASSAGELAB_GRAINIENT.warpFrequency, 0.1, 20),
    warpSpeed: clampNumber(options?.warpSpeed, DEFAULT_MASSAGELAB_GRAINIENT.warpSpeed, 0, 6),
    warpAmplitude: clampNumber(options?.warpAmplitude, DEFAULT_MASSAGELAB_GRAINIENT.warpAmplitude, 1, 160),
    blendAngle: clampNumber(options?.blendAngle, DEFAULT_MASSAGELAB_GRAINIENT.blendAngle, -180, 180),
    blendSoftness: clampNumber(options?.blendSoftness, DEFAULT_MASSAGELAB_GRAINIENT.blendSoftness, 0, 1),
    rotationAmount: clampNumber(options?.rotationAmount, DEFAULT_MASSAGELAB_GRAINIENT.rotationAmount, 0, 1200),
    noiseScale: clampNumber(options?.noiseScale, DEFAULT_MASSAGELAB_GRAINIENT.noiseScale, 0.1, 8),
    grainAmount: clampNumber(options?.grainAmount, DEFAULT_MASSAGELAB_GRAINIENT.grainAmount, 0, 1),
    grainScale: clampNumber(options?.grainScale, DEFAULT_MASSAGELAB_GRAINIENT.grainScale, 0.1, 12),
    grainAnimated: options?.grainAnimated === true,
    contrast: clampNumber(options?.contrast, DEFAULT_MASSAGELAB_GRAINIENT.contrast, 0.2, 4),
    gamma: clampNumber(options?.gamma, DEFAULT_MASSAGELAB_GRAINIENT.gamma, 0.2, 4),
    saturation: clampNumber(options?.saturation, DEFAULT_MASSAGELAB_GRAINIENT.saturation, 0, 3),
    centerX: clampNumber(options?.centerX, DEFAULT_MASSAGELAB_GRAINIENT.centerX, -1, 1),
    centerY: clampNumber(options?.centerY, DEFAULT_MASSAGELAB_GRAINIENT.centerY, -1, 1),
    zoom: clampNumber(options?.zoom, DEFAULT_MASSAGELAB_GRAINIENT.zoom, 0.2, 3),
    color1: normalizeHexColor(options?.color1, DEFAULT_MASSAGELAB_GRAINIENT.color1),
    color2: normalizeHexColor(options?.color2, DEFAULT_MASSAGELAB_GRAINIENT.color2),
    color3: normalizeHexColor(options?.color3, DEFAULT_MASSAGELAB_GRAINIENT.color3),
  }
}
