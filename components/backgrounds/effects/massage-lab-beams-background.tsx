...
import type { BackgroundEffectProps, MassageLabBeamsOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedBeamsOptions = Required<MassageLabBeamsOptions>
...
const DEFAULT_MASSAGELAB_BEAMS: ResolvedBeamsOptions = {
  beamWidth: 2,
  beamHeight: 15,
  beamNumber: 12,
  lightColor: "#FFFFFF",
...
export default function MassageLabBeamsBackground({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabBeams, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabBeamsCanvas} />
    </div>
  )
}
...
function resolveBeamsOptions(options: MassageLabBeamsOptions | undefined): ResolvedBeamsOptions {
  return {
    beamWidth: resolveNumber(options?.beamWidth, DEFAULT_MASSAGELAB_BEAMS.beamWidth, 0.2, 6),
    beamHeight: resolveNumber(options?.beamHeight, DEFAULT_MASSAGELAB_BEAMS.beamHeight, 4, 32),
    beamNumber: Math.trunc(resolveNumber(options?.beamNumber, DEFAULT_MASSAGELAB_BEAMS.beamNumber, 1, 48)),
    lightColor: resolveHexColor(options?.lightColor, DEFAULT_MASSAGELAB_BEAMS.lightColor),
    speed: resolveNumber(options?.speed, DEFAULT_MASSAGELAB_BEAMS.speed, 0, 8),
    noiseIntensity: resolveNumber(
      options?.noiseIntensity,
      DEFAULT_MASSAGELAB_BEAMS.noiseIntensity,
      0,
      4,
    ),
    scale: resolveNumber(options?.scale, DEFAULT_MASSAGELAB_BEAMS.scale, 0.02, 1.5),
    rotation: resolveNumber(options?.rotation, DEFAULT_MASSAGELAB_BEAMS.rotation, -180, 180),
  }
}

function parseHexColorToRgb(color: string): RgbColor {
  const normalized = resolveHexColor(color, DEFAULT_MASSAGELAB_BEAMS.lightColor).replace("#", "")
  return [
