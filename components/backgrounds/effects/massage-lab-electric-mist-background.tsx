...
import type { BackgroundEffectProps, MassageLabElectricMistOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]

type ResolvedElectricMistOptions = Required<MassageLabElectricMistOptions>
...
export default function MassageLabElectricMistBackground({
  ...
  const color = massageLabElectricMist?.color
  const speed = massageLabElectricMist?.speed
  const detail = massageLabElectricMist?.detail
  const distortion = massageLabElectricMist?.distortion
  const brightness = massageLabElectricMist?.brightness
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabElectricMist, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabElectricMistCanvas} />
    </div>
  )
}
