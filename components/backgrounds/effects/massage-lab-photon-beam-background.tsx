...
import type { BackgroundEffectProps, MassageLabPhotonBeamOptions } from "./css-backgrounds"
...
type ResolvedPhotonBeamOptions = Required<MassageLabPhotonBeamOptions>

const DEFAULT_MASSAGE_LAB_PHOTON_BEAM: ResolvedPhotonBeamOptions = {
  colorBg: "#080808",
  colorLine: "#005F6F",
  colorSignal: "#00D9FF",
  useColor2: false,
...
export default function MassageLabPhotonBeamBackground({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabPhotonBeamBackground, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabPhotonBeamCanvas} />
    </div>
  )
}

function resolvePhotonBeamOptions(options: MassageLabPhotonBeamOptions): ResolvedPhotonBeamOptions {
  return {
    colorBg: normalizeHexColor(options.colorBg, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.colorBg),
    colorLine: normalizeHexColor(options.colorLine, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.colorLine),
    colorSignal: normalizeHexColor(options.colorSignal, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.colorSignal),
    useColor2: typeof options.useColor2 === "boolean" ? options.useColor2 : DEFAULT_MASSAGE_LAB_PHOTON_BEAM.useColor2,
    colorSignal2: normalizeHexColor(options.colorSignal2, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.colorSignal2),
    useColor3: typeof options.useColor3 === "boolean" ? options.useColor3 : DEFAULT_MASSAGE_LAB_PHOTON_BEAM.useColor3,
    colorSignal3: normalizeHexColor(options.colorSignal3, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.colorSignal3),
    lineCount: Math.round(clampNumber(options.lineCount, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.lineCount, 12, 160)),
    spreadHeight: clampNumber(options.spreadHeight, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.spreadHeight, 5, 90),
    spreadDepth: clampNumber(options.spreadDepth, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.spreadDepth, 0, 60),
    curveLength: clampNumber(options.curveLength, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.curveLength, 16, 120),
    straightLength: clampNumber(options.straightLength, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.straightLength, 40, 220),
    curvePower: clampNumber(options.curvePower, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.curvePower, 0.2, 2),
    waveSpeed: clampNumber(options.waveSpeed, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.waveSpeed, 0, 8),
    waveHeight: clampNumber(options.waveHeight, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.waveHeight, 0, 1),
    lineOpacity: clampNumber(options.lineOpacity, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.lineOpacity, 0.05, 1),
    signalCount: Math.round(clampNumber(options.signalCount, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.signalCount, 0, 220)),
    speedGlobal: clampNumber(options.speedGlobal, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.speedGlobal, 0.02, 2),
    trailLength: Math.round(clampNumber(options.trailLength, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.trailLength, 1, 16)),
    bloomStrength: clampNumber(options.bloomStrength, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.bloomStrength, 0, 6),
    bloomRadius: clampNumber(options.bloomRadius, DEFAULT_MASSAGE_LAB_PHOTON_BEAM.bloomRadius, 0, 1.5),
  }
}
