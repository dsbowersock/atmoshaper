...
import type { BackgroundEffectProps, MassageLabEvilEyeOptions } from "./css-backgrounds"

type ResolvedEvilEyeOptions = Required<MassageLabEvilEyeOptions>
...
const DEFAULT_MASSAGELAB_EVIL_EYE: ResolvedEvilEyeOptions = {
  eyeColor: "#FF6F37",
  intensity: 1.5,
  pupilSize: 0.6,
  irisWidth: 0.25,
...
export default function MassageLabEvilEyeBackground({
  ...
  const eyeColor = massageLabEvilEye?.eyeColor
  const intensity = massageLabEvilEye?.intensity
  const pupilSize = massageLabEvilEye?.pupilSize
  const irisWidth = massageLabEvilEye?.irisWidth
  const glowIntensity = massageLabEvilEye?.glowIntensity
  const scale = massageLabEvilEye?.scale
  const noiseScale = massageLabEvilEye?.noiseScale
  const pupilFollow = massageLabEvilEye?.pupilFollow
  const flameSpeed = massageLabEvilEye?.flameSpeed
  const backgroundColor = massageLabEvilEye?.backgroundColor
  const interactive = massageLabEvilEye?.interactive
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabEvilEye, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabEvilEyeCanvas} />
    </div>
  )
}
...
function resolveEvilEyeOptions(options: MassageLabEvilEyeOptions | undefined): ResolvedEvilEyeOptions {
  return {
    eyeColor: normalizeHexColor(options?.eyeColor, DEFAULT_MASSAGELAB_EVIL_EYE.eyeColor),
    intensity: clampNumber(options?.intensity, DEFAULT_MASSAGELAB_EVIL_EYE.intensity, 0, 3),
    pupilSize: clampNumber(options?.pupilSize, DEFAULT_MASSAGELAB_EVIL_EYE.pupilSize, 0.1, 2),
    irisWidth: clampNumber(options?.irisWidth, DEFAULT_MASSAGELAB_EVIL_EYE.irisWidth, 0.05, 1),
    glowIntensity: clampNumber(options?.glowIntensity, DEFAULT_MASSAGELAB_EVIL_EYE.glowIntensity, 0, 1.5),
    scale: clampNumber(options?.scale, DEFAULT_MASSAGELAB_EVIL_EYE.scale, 0.25, 2),
    noiseScale: clampNumber(options?.noiseScale, DEFAULT_MASSAGELAB_EVIL_EYE.noiseScale, 0.1, 4),
    pupilFollow: clampNumber(options?.pupilFollow, DEFAULT_MASSAGELAB_EVIL_EYE.pupilFollow, 0, 2),
    flameSpeed: clampNumber(options?.flameSpeed, DEFAULT_MASSAGELAB_EVIL_EYE.flameSpeed, 0, 3),
    backgroundColor: normalizeHexColor(options?.backgroundColor, DEFAULT_MASSAGELAB_EVIL_EYE.backgroundColor),
    interactive: typeof options?.interactive === "boolean" ? options.interactive : DEFAULT_MASSAGELAB_EVIL_EYE.interactive,
  }
}
