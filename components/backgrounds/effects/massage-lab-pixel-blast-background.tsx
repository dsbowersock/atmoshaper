...
import type { BackgroundEffectProps, MassageLabPixelBlastOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type PixelBlastVariant = NonNullable<MassageLabPixelBlastOptions["variant"]>
type ResolvedPixelBlastOptions = Required<MassageLabPixelBlastOptions>
...
const DEFAULT_MASSAGELAB_PIXEL_BLAST: ResolvedPixelBlastOptions = {
  variant: "square",
  pixelSize: 3,
  color: "#B497CF",
  antialias: true,
...
export default function MassageLabPixelBlastBackground({
  ...
  const options = useMemo(
    () => resolvePixelBlastOptions(massageLabPixelBlast),
    [massageLabPixelBlast],
  )
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabPixelBlast, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabPixelBlastCanvas} />
    </div>
  )
}
...
function resolvePixelBlastOptions(
  options: MassageLabPixelBlastOptions | undefined,
): ResolvedPixelBlastOptions {
  return {
    variant: resolveVariant(options?.variant),
    pixelSize: resolveNumber(options?.pixelSize, DEFAULT_MASSAGELAB_PIXEL_BLAST.pixelSize, 1, 16),
    color: resolveHexColor(options?.color, DEFAULT_MASSAGELAB_PIXEL_BLAST.color),
    antialias: resolveBoolean(options?.antialias, DEFAULT_MASSAGELAB_PIXEL_BLAST.antialias),
    patternScale: resolveNumber(options?.patternScale, DEFAULT_MASSAGELAB_PIXEL_BLAST.patternScale, 0.25, 8),
    patternDensity: resolveNumber(options?.patternDensity, DEFAULT_MASSAGELAB_PIXEL_BLAST.patternDensity, 0, 2),
    liquid: resolveBoolean(options?.liquid, DEFAULT_MASSAGELAB_PIXEL_BLAST.liquid),
    liquidStrength: resolveNumber(options?.liquidStrength, DEFAULT_MASSAGELAB_PIXEL_BLAST.liquidStrength, 0, 0.4),
    liquidRadius: resolveNumber(options?.liquidRadius, DEFAULT_MASSAGELAB_PIXEL_BLAST.liquidRadius, 0.1, 4),
    pixelSizeJitter: resolveNumber(
      options?.pixelSizeJitter,
      DEFAULT_MASSAGELAB_PIXEL_BLAST.pixelSizeJitter,
      0,
      1,
    ),
    enableRipples: resolveBoolean(options?.enableRipples, DEFAULT_MASSAGELAB_PIXEL_BLAST.enableRipples),
    rippleIntensityScale: resolveNumber(
      options?.rippleIntensityScale,
      DEFAULT_MASSAGELAB_PIXEL_BLAST.rippleIntensityScale,
      0,
      4,
    ),
    rippleThickness: resolveNumber(
      options?.rippleThickness,
      DEFAULT_MASSAGELAB_PIXEL_BLAST.rippleThickness,
      0.01,
      0.5,
    ),
    rippleSpeed: resolveNumber(options?.rippleSpeed, DEFAULT_MASSAGELAB_PIXEL_BLAST.rippleSpeed, 0.05, 2),
    liquidWobbleSpeed: resolveNumber(
      options?.liquidWobbleSpeed,
      DEFAULT_MASSAGELAB_PIXEL_BLAST.liquidWobbleSpeed,
      0,
      10,
    ),
    autoPauseOffscreen: resolveBoolean(
      options?.autoPauseOffscreen,
      DEFAULT_MASSAGELAB_PIXEL_BLAST.autoPauseOffscreen,
    ),
    speed: resolveNumber(options?.speed, DEFAULT_MASSAGELAB_PIXEL_BLAST.speed, 0, 3),
    transparent: resolveBoolean(options?.transparent, DEFAULT_MASSAGELAB_PIXEL_BLAST.transparent),
    edgeFade: resolveNumber(options?.edgeFade, DEFAULT_MASSAGELAB_PIXEL_BLAST.edgeFade, 0, 1),
    noiseAmount: resolveNumber(options?.noiseAmount, DEFAULT_MASSAGELAB_PIXEL_BLAST.noiseAmount, 0, 0.4),
  }
}

function resolveVariant(value: MassageLabPixelBlastOptions["variant"] | undefined): PixelBlastVariant {
  if (value === "square" || value === "circle" || value === "triangle" || value === "diamond") {
    return value
  }

  return DEFAULT_MASSAGELAB_PIXEL_BLAST.variant
}
...
function parseHexColorToLinearRgb(color: string): RgbColor {
  const normalized = resolveHexColor(color, DEFAULT_MASSAGELAB_PIXEL_BLAST.color).replace("#", "")
  return [
