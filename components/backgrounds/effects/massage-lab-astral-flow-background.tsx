...
import type { BackgroundEffectProps, MassageLabAstralFlowOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]

type ResolvedAstralFlowOptions = Required<MassageLabAstralFlowOptions>
...
const DEFAULT_MASSAGE_LAB_ASTRAL_FLOW: ResolvedAstralFlowOptions = {
  color1: "#05070A",
  color2: "#2E1A38",
  color3: "#A0769A",
  speed: 1.5,
...
// AtmoShaper Astral Flow ships as a Three/R3F shader component. This keeps the
// same shader controls in AtmoShaper without adding that dependency stack.
export default function MassageLabAstralFlowBackground({
  className,
  massageLabAstralFlow,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const color1 = massageLabAstralFlow?.color1
  const color2 = massageLabAstralFlow?.color2
  const color3 = massageLabAstralFlow?.color3
  const speed = massageLabAstralFlow?.speed
  const flowMin = massageLabAstralFlow?.flowMin
  const flowMax = massageLabAstralFlow?.flowMax
  ...
  useEffect(() => {
    ...
    const drawFrame = (timestamp: number, animate: boolean) => {
      ...
      // Match the reviewed WebGL source, where shader time starts when the canvas mounts.
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabAstralFlow, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabAstralFlowCanvas} />
      <div className={styles.massageLabAstralFlowNoise} />
    </div>
  )
}
...
function getUniformLocation(context: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (location === null) {
    throw new Error(`Missing ${PUBLIC_PRODUCT_IDENTITY.name} Astral Flow shader uniform: ${name}`)
  }

  return location
}

function resolveAstralFlowOptions(options: MassageLabAstralFlowOptions): ResolvedAstralFlowOptions {
  const flowMin = clampNumber(options.flowMin, DEFAULT_MASSAGE_LAB_ASTRAL_FLOW.flowMin, 0.5, 10)
  const flowMax = clampNumber(options.flowMax, DEFAULT_MASSAGE_LAB_ASTRAL_FLOW.flowMax, 1, 12)

  return {
    color1: normalizeHexColor(options.color1, DEFAULT_MASSAGE_LAB_ASTRAL_FLOW.color1),
    color2: normalizeHexColor(options.color2, DEFAULT_MASSAGE_LAB_ASTRAL_FLOW.color2),
    color3: normalizeHexColor(options.color3, DEFAULT_MASSAGE_LAB_ASTRAL_FLOW.color3),
    speed: clampNumber(options.speed, DEFAULT_MASSAGE_LAB_ASTRAL_FLOW.speed, 0.1, 3),
