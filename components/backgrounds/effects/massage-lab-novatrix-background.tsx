...
import type { BackgroundEffectProps, MassageLabNovatrixOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]

type ResolvedNovatrixOptions = Required<MassageLabNovatrixOptions>
...
const DEFAULT_MASSAGE_LAB_NOVATRIX: ResolvedNovatrixOptions = {
  color: "#FFFFFF",
  speed: 1,
  amplitude: 0.1,
}
...
export default function MassageLabNovatrixBackground({
  ...
  const color = massageLabNovatrix?.color
  const speed = massageLabNovatrix?.speed
  const amplitude = massageLabNovatrix?.amplitude
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabNovatrixBackground, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabNovatrixCanvas} />
    </div>
  )
}
...
function resolveNovatrixOptions(options: MassageLabNovatrixOptions): ResolvedNovatrixOptions {
  return {
    color: normalizeHexColor(options.color, DEFAULT_MASSAGE_LAB_NOVATRIX.color),
    speed: clampNumber(options.speed, DEFAULT_MASSAGE_LAB_NOVATRIX.speed, 0.02, 3),
    amplitude: clampNumber(options.amplitude, DEFAULT_MASSAGE_LAB_NOVATRIX.amplitude, 0.01, 0.45),
  }
}
