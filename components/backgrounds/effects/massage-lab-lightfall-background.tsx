...
import type { BackgroundEffectProps, MassageLabLightfallOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedLightfallOptions = Required<Omit<MassageLabLightfallOptions, "colors">> & {
  colors: string[]
}
...
const DEFAULT_MASSAGELAB_LIGHTFALL: ResolvedLightfallOptions = {
...
export default function MassageLabLightfallBackground({
  ...
  const colorsKey = (massageLabLightfall?.colors ?? []).join("|")
  const backgroundColor = massageLabLightfall?.backgroundColor
  const speed = massageLabLightfall?.speed
  const streakCount = massageLabLightfall?.streakCount
  const streakWidth = massageLabLightfall?.streakWidth
  const streakLength = massageLabLightfall?.streakLength
  const glow = massageLabLightfall?.glow
  const density = massageLabLightfall?.density
  const twinkle = massageLabLightfall?.twinkle
  const zoom = massageLabLightfall?.zoom
  const backgroundGlow = massageLabLightfall?.backgroundGlow
  const opacity = massageLabLightfall?.opacity
  const mouseInteraction = massageLabLightfall?.mouseInteraction
  const mouseStrength = massageLabLightfall?.mouseStrength
  const mouseRadius = massageLabLightfall?.mouseRadius
  const mouseDampening = massageLabLightfall?.mouseDampening
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabLightfall, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabLightfallCanvas} />
    </div>
  )
}
...
function resolveLightfallOptions(options: MassageLabLightfallOptions | undefined): ResolvedLightfallOptions {
  return {
    colors: resolveLightfallColors(options?.colors),
    backgroundColor: normalizeHexColor(options?.backgroundColor, DEFAULT_MASSAGELAB_LIGHTFALL.backgroundColor),
    speed: clampNumber(options?.speed, DEFAULT_MASSAGELAB_LIGHTFALL.speed, 0.05, 2),
    streakCount: Math.trunc(clampNumber(options?.streakCount, DEFAULT_MASSAGELAB_LIGHTFALL.streakCount, 1, 16)),
    streakWidth: clampNumber(options?.streakWidth, DEFAULT_MASSAGELAB_LIGHTFALL.streakWidth, 0.2, 3),
    streakLength: clampNumber(options?.streakLength, DEFAULT_MASSAGELAB_LIGHTFALL.streakLength, 0.2, 3),
    glow: clampNumber(options?.glow, DEFAULT_MASSAGELAB_LIGHTFALL.glow, 0.1, 3),
    density: clampNumber(options?.density, DEFAULT_MASSAGELAB_LIGHTFALL.density, 0.05, 2),
    twinkle: clampNumber(options?.twinkle, DEFAULT_MASSAGELAB_LIGHTFALL.twinkle, 0, 1),
    zoom: clampNumber(options?.zoom, DEFAULT_MASSAGELAB_LIGHTFALL.zoom, 1, 6),
    backgroundGlow: clampNumber(options?.backgroundGlow, DEFAULT_MASSAGELAB_LIGHTFALL.backgroundGlow, 0, 1.5),
    opacity: clampNumber(options?.opacity, DEFAULT_MASSAGELAB_LIGHTFALL.opacity, 0.05, 1),
    mouseInteraction: options?.mouseInteraction === true,
    mouseStrength: clampNumber(options?.mouseStrength, DEFAULT_MASSAGELAB_LIGHTFALL.mouseStrength, 0, 2),
    mouseRadius: clampNumber(options?.mouseRadius, DEFAULT_MASSAGELAB_LIGHTFALL.mouseRadius, 0.05, 3),
    mouseDampening: clampNumber(options?.mouseDampening, DEFAULT_MASSAGELAB_LIGHTFALL.mouseDampening, 0, 1),
  }
}
...
function resolveLightfallColors(colors: string[] | undefined) {
  ...
  return sanitized.length ? sanitized : DEFAULT_MASSAGELAB_LIGHTFALL.colors
}
...
function prepLightfallColors(colors: string[]) {
  ...
  for (let index = 0; index < MAX_LIGHTFALL_COLORS; index += 1) {
    arr.push(parseHexColorToRgb(base[Math.min(index, base.length - 1)] ?? DEFAULT_MASSAGELAB_LIGHTFALL.colors[0]))
  }
