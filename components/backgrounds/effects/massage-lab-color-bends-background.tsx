...
import type { BackgroundEffectProps, MassageLabColorBendsOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedColorBendsOptions = Required<MassageLabColorBendsOptions>
...
const DEFAULT_MASSAGELAB_COLOR_BENDS: ResolvedColorBendsOptions = {
  rotation: 90,
  speed: 0.2,
  colors: [],
  transparent: true,
...
export default function MassageLabColorBendsBackground({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabColorBends, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabColorBendsCanvas} />
    </div>
  )
}
...
function resolveColorBendsOptions(options?: MassageLabColorBendsOptions): ResolvedColorBendsOptions {
  return {
    rotation: clampNumber(options?.rotation, DEFAULT_MASSAGELAB_COLOR_BENDS.rotation, -360, 360),
    speed: clampNumber(options?.speed, DEFAULT_MASSAGELAB_COLOR_BENDS.speed, 0, 3),
    colors: Array.isArray(options?.colors) ? options.colors.filter(isHexColor).slice(0, MAX_COLORS) : [],
    transparent: typeof options?.transparent === "boolean" ? options.transparent : DEFAULT_MASSAGELAB_COLOR_BENDS.transparent,
    autoRotate: clampNumber(options?.autoRotate, DEFAULT_MASSAGELAB_COLOR_BENDS.autoRotate, -180, 180),
    scale: clampNumber(options?.scale, DEFAULT_MASSAGELAB_COLOR_BENDS.scale, 0.1, 4),
    frequency: clampNumber(options?.frequency, DEFAULT_MASSAGELAB_COLOR_BENDS.frequency, 0.1, 4),
    warpStrength: clampNumber(options?.warpStrength, DEFAULT_MASSAGELAB_COLOR_BENDS.warpStrength, 0, 3),
    mouseInfluence: clampNumber(options?.mouseInfluence, DEFAULT_MASSAGELAB_COLOR_BENDS.mouseInfluence, 0, 3),
    parallax: clampNumber(options?.parallax, DEFAULT_MASSAGELAB_COLOR_BENDS.parallax, 0, 2),
    noise: clampNumber(options?.noise, DEFAULT_MASSAGELAB_COLOR_BENDS.noise, 0, 1),
    iterations: clampInteger(options?.iterations, DEFAULT_MASSAGELAB_COLOR_BENDS.iterations, 1, 5),
    intensity: clampNumber(options?.intensity, DEFAULT_MASSAGELAB_COLOR_BENDS.intensity, 0.1, 4),
    bandWidth: clampNumber(options?.bandWidth, DEFAULT_MASSAGELAB_COLOR_BENDS.bandWidth, 0.5, 16),
    interactive: typeof options?.interactive === "boolean" ? options.interactive : DEFAULT_MASSAGELAB_COLOR_BENDS.interactive,
  }
}
