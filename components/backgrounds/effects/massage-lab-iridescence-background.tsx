...
import type { BackgroundEffectProps, MassageLabIridescenceOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedIridescenceOptions = Required<MassageLabIridescenceOptions>
...
const DEFAULT_MASSAGELAB_IRIDESCENCE: ResolvedIridescenceOptions = {
  color: "#FFFFFF",
  speed: 1,
  amplitude: 0.1,
  mouseReact: true,
...
export default function MassageLabIridescenceBackground({
  ...
  const options = useMemo(
    () => resolveIridescenceOptions(massageLabIridescence),
    [massageLabIridescence],
  )
  ...
  return (
    <div className={cn(styles.effectLayer, styles.massageLabIridescence, className)} ref={containerRef}>
      <canvas className={styles.massageLabIridescenceCanvas} ref={canvasRef} />
    </div>
  )
}
...
function resolveIridescenceOptions(options?: MassageLabIridescenceOptions): ResolvedIridescenceOptions {
  return {
    color: normalizeHexColor(options?.color, DEFAULT_MASSAGELAB_IRIDESCENCE.color),
    speed: clampNumber(options?.speed, DEFAULT_MASSAGELAB_IRIDESCENCE.speed, 0, 3),
    amplitude: clampNumber(options?.amplitude, DEFAULT_MASSAGELAB_IRIDESCENCE.amplitude, 0, 1),
    mouseReact: typeof options?.mouseReact === "boolean"
      ? options.mouseReact
      : DEFAULT_MASSAGELAB_IRIDESCENCE.mouseReact,
  }
}
