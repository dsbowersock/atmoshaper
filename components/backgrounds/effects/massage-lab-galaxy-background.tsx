...
import type { BackgroundEffectProps, MassageLabGalaxyOptions } from "./css-backgrounds"

type ResolvedGalaxyOptions = Required<MassageLabGalaxyOptions>
...
const DEFAULT_MASSAGELAB_GALAXY: ResolvedGalaxyOptions = {
  focalX: 0.5,
  focalY: 0.5,
  rotationDeg: 0,
  starSpeed: 0.5,
...
export default function MassageLabGalaxyBackground({
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.massageLabGalaxyCanvas, className)}
    />
  )
}
...
function resolveGalaxyOptions(options: MassageLabGalaxyOptions | undefined): ResolvedGalaxyOptions {
  return {
    focalX: resolveNumber(options?.focalX, DEFAULT_MASSAGELAB_GALAXY.focalX, 0, 1),
    focalY: resolveNumber(options?.focalY, DEFAULT_MASSAGELAB_GALAXY.focalY, 0, 1),
    rotationDeg: resolveNumber(options?.rotationDeg, DEFAULT_MASSAGELAB_GALAXY.rotationDeg, -360, 360),
    starSpeed: resolveNumber(options?.starSpeed, DEFAULT_MASSAGELAB_GALAXY.starSpeed, 0, 5),
    density: resolveNumber(options?.density, DEFAULT_MASSAGELAB_GALAXY.density, 0.1, 3),
    hueShift: resolveNumber(options?.hueShift, DEFAULT_MASSAGELAB_GALAXY.hueShift, 0, 360),
    speed: resolveNumber(options?.speed, DEFAULT_MASSAGELAB_GALAXY.speed, 0, 5),
    mouseInteraction: typeof options?.mouseInteraction === "boolean"
      ? options.mouseInteraction
      : DEFAULT_MASSAGELAB_GALAXY.mouseInteraction,
    glowIntensity: resolveNumber(options?.glowIntensity, DEFAULT_MASSAGELAB_GALAXY.glowIntensity, 0.01, 2),
    saturation: resolveNumber(options?.saturation, DEFAULT_MASSAGELAB_GALAXY.saturation, 0, 2),
    mouseRepulsion: typeof options?.mouseRepulsion === "boolean"
      ? options.mouseRepulsion
      : DEFAULT_MASSAGELAB_GALAXY.mouseRepulsion,
    repulsionStrength: resolveNumber(
      options?.repulsionStrength,
      DEFAULT_MASSAGELAB_GALAXY.repulsionStrength,
      0,
      6,
    ),
    twinkleIntensity: resolveNumber(
      options?.twinkleIntensity,
      DEFAULT_MASSAGELAB_GALAXY.twinkleIntensity,
      0,
      1,
    ),
    rotationSpeed: resolveNumber(options?.rotationSpeed, DEFAULT_MASSAGELAB_GALAXY.rotationSpeed, -2, 2),
    autoCenterRepulsion: resolveNumber(
      options?.autoCenterRepulsion,
      DEFAULT_MASSAGELAB_GALAXY.autoCenterRepulsion,
      0,
      6,
    ),
    transparent: typeof options?.transparent === "boolean"
      ? options.transparent
      : DEFAULT_MASSAGELAB_GALAXY.transparent,
  }
}
