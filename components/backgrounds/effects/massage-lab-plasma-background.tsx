...
import type { BackgroundEffectProps, MassageLabPlasmaOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedPlasmaOptions = Required<MassageLabPlasmaOptions>
...
const DEFAULT_MASSAGELAB_PLASMA: ResolvedPlasmaOptions = {
  color: "#FFFFFF",
  speed: 1,
  direction: "forward",
  scale: 1,
...
export default function MassageLabPlasmaBackground({
  ...
  const options = useMemo(
    () => resolvePlasmaOptions(massageLabPlasma),
    [massageLabPlasma],
  )
  ...
  return (
    <div className={cn(styles.effectLayer, styles.massageLabPlasma, className)} ref={containerRef}>
      <canvas className={styles.massageLabPlasmaCanvas} ref={canvasRef} />
    </div>
  )
}
...
function resolvePlasmaOptions(options?: MassageLabPlasmaOptions): ResolvedPlasmaOptions {
  return {
    color: normalizeHexColor(options?.color, DEFAULT_MASSAGELAB_PLASMA.color),
    speed: clampNumber(options?.speed, DEFAULT_MASSAGELAB_PLASMA.speed, 0, 3),
    direction: resolveDirection(options?.direction),
    scale: clampNumber(options?.scale, DEFAULT_MASSAGELAB_PLASMA.scale, 0.2, 4),
    opacity: clampNumber(options?.opacity, DEFAULT_MASSAGELAB_PLASMA.opacity, 0, 1),
    mouseInteractive: typeof options?.mouseInteractive === "boolean"
      ? options.mouseInteractive
      : DEFAULT_MASSAGELAB_PLASMA.mouseInteractive,
  }
}
