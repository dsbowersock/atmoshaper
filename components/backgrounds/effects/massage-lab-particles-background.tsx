...
import type { BackgroundEffectProps, MassageLabParticlesOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedParticlesOptions = Omit<Required<MassageLabParticlesOptions>, "colors"> & {
...
const DEFAULT_MASSAGELAB_PARTICLES: ResolvedParticlesOptions = {
  particleCount: 200,
  particleSpread: 10,
  speed: 0.1,
...
export default function MassageLabParticlesBackground({
  className,
  massageLabParticles,
  ...
  const options = useMemo(
    () => resolveParticlesOptions(massageLabParticles),
    [massageLabParticles],
  )
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.massageLabParticlesCanvas, className)}
    />
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing ${PUBLIC_PRODUCT_IDENTITY.name} Particles uniform: ${name}`)
  }
...
function resolveParticlesOptions(options?: MassageLabParticlesOptions): ResolvedParticlesOptions {
  return {
    particleCount: Math.round(clampNumber(options?.particleCount, DEFAULT_MASSAGELAB_PARTICLES.particleCount, 20, 1500)),
    particleSpread: clampNumber(options?.particleSpread, DEFAULT_MASSAGELAB_PARTICLES.particleSpread, 1, 30),
    speed: clampNumber(options?.speed, DEFAULT_MASSAGELAB_PARTICLES.speed, 0, 1),
    colors: normalizeColorList(options?.colors, DEFAULT_MASSAGELAB_PARTICLES.colors),
    moveParticlesOnHover: options?.moveParticlesOnHover === true,
    particleHoverFactor: clampNumber(
      options?.particleHoverFactor,
      DEFAULT_MASSAGELAB_PARTICLES.particleHoverFactor,
      0,
      5,
    ),
    alphaParticles: options?.alphaParticles === true,
    particleBaseSize: clampNumber(options?.particleBaseSize, DEFAULT_MASSAGELAB_PARTICLES.particleBaseSize, 10, 300),
    sizeRandomness: clampNumber(options?.sizeRandomness, DEFAULT_MASSAGELAB_PARTICLES.sizeRandomness, 0, 3),
    cameraDistance: clampNumber(options?.cameraDistance, DEFAULT_MASSAGELAB_PARTICLES.cameraDistance, 5, 60),
    disableRotation: options?.disableRotation === true,
    pixelRatio: clampNumber(options?.pixelRatio, DEFAULT_MASSAGELAB_PARTICLES.pixelRatio, 0.5, 2),
  }
}
