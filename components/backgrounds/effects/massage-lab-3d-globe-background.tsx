...
import type { MassageLab3DGlobeOptions, BackgroundEffectProps } from "./css-backgrounds"
...
function resolveGlobeOptions(options: MassageLab3DGlobeOptions | undefined): ResolvedGlobeOptions {
  return {
...
export default function MassageLab3DGlobeBackground({
  className,
  massageLab3DGlobe,
  ...
  const optionAmbientIntensity = massageLab3DGlobe?.ambientIntensity
  const optionAtmosphereBlur = massageLab3DGlobe?.atmosphereBlur
  const optionAtmosphereColor = massageLab3DGlobe?.atmosphereColor
  const optionAtmosphereIntensity = massageLab3DGlobe?.atmosphereIntensity
  const optionAutoRotateSpeed = massageLab3DGlobe?.autoRotateSpeed
  const optionBackgroundColor = massageLab3DGlobe?.backgroundColor
  const optionBumpScale = massageLab3DGlobe?.bumpScale
  const optionGlobeColor = massageLab3DGlobe?.globeColor
  const optionGlobeScale = massageLab3DGlobe?.globeScale
  const optionGraphicGlowColor = massageLab3DGlobe?.graphicGlowColor
  const optionGraphicMapColor = massageLab3DGlobe?.graphicMapColor
  const optionGraphicMapSamples = massageLab3DGlobe?.graphicMapSamples
  const optionGraphicMarkerColor = massageLab3DGlobe?.graphicMarkerColor
  const optionEnablePan = massageLab3DGlobe?.enablePan
  const optionLightingMode = massageLab3DGlobe?.lightingMode
  const optionPanX = massageLab3DGlobe?.panX
  const optionPanY = massageLab3DGlobe?.panY
  const optionShowTilt = massageLab3DGlobe?.showTilt
  const optionMarkerEnabled = massageLab3DGlobe?.markerEnabled
  const optionMarkerIcon = massageLab3DGlobe?.markerIcon
  const optionMarkerLabel = massageLab3DGlobe?.markerLabel
  const optionMarkerLat = massageLab3DGlobe?.markerLat
  const optionMarkerLng = massageLab3DGlobe?.markerLng
  const optionMarkerSize = massageLab3DGlobe?.markerSize
  const optionPointLightIntensity = massageLab3DGlobe?.pointLightIntensity
  const optionReverseSpin = massageLab3DGlobe?.reverseSpin
  const optionShowAtmosphere = massageLab3DGlobe?.showAtmosphere
  const optionShowWireframe = massageLab3DGlobe?.showWireframe
  const optionViewStyle = massageLab3DGlobe?.viewStyle
  const optionWireframeColor = massageLab3DGlobe?.wireframeColor
  ...
  const style = {
    "--ml-globe-background": resolved.backgroundColor,
  } as CSSProperties
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLab3dGlobe, className)}
      style={style}
      aria-hidden="true"
    >
      <canvas ref={globeCanvasRef} className={styles.massageLab3dGlobeCanvas} />
      <canvas ref={markerCanvasRef} className={styles.massageLab3dGlobeMarkerCanvas} />
    </div>
  )
}
