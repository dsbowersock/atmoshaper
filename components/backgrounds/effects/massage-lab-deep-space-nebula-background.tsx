...
import type { BackgroundEffectProps, MassageLabDeepSpaceNebulaOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]

type ResolvedDeepSpaceNebulaOptions = Required<MassageLabDeepSpaceNebulaOptions>
...
const DEFAULT_MASSAGE_LAB_DEEP_SPACE_NEBULA: ResolvedDeepSpaceNebulaOptions = {
  color1: "#5EFFF4",
  color2: "#763B65",
  color3: "#1A0B2E",
  speed: 2,
...
// AtmoShaper publishes Nebula as a Three/R3F shader. AtmoShaper keeps the
// source prop model but renders it through native WebGL to avoid that stack.
export default function MassageLabDeepSpaceNebulaBackground({
  className,
  massageLabDeepSpaceNebula,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const color1 = massageLabDeepSpaceNebula?.color1
  const color2 = massageLabDeepSpaceNebula?.color2
  const color3 = massageLabDeepSpaceNebula?.color3
  const speed = massageLabDeepSpaceNebula?.speed
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabDeepSpaceNebula, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabDeepSpaceNebulaCanvas} />
    </div>
  )
}
...
function getUniformLocation(context: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (location === null) {
    throw new Error(`Missing ${PUBLIC_PRODUCT_IDENTITY.name} Deep Space Nebula shader uniform: ${name}`)
  }
...
function resolveDeepSpaceNebulaOptions(
  options: MassageLabDeepSpaceNebulaOptions,
): ResolvedDeepSpaceNebulaOptions {
  return {
    color1: normalizeHexColor(options.color1, DEFAULT_MASSAGE_LAB_DEEP_SPACE_NEBULA.color1),
    color2: normalizeHexColor(options.color2, DEFAULT_MASSAGE_LAB_DEEP_SPACE_NEBULA.color2),
    color3: normalizeHexColor(options.color3, DEFAULT_MASSAGE_LAB_DEEP_SPACE_NEBULA.color3),
    speed: clampNumber(options.speed, DEFAULT_MASSAGE_LAB_DEEP_SPACE_NEBULA.speed, 0.1, 5),
  }
}
