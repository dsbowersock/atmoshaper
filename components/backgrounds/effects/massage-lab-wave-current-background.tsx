...
import type { BackgroundEffectProps, MassageLabWaveCurrentOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]

type ResolvedWavesOptions = Required<MassageLabWaveCurrentOptions>
...
const DEFAULT_MASSAGE_LAB_WAVES: ResolvedWavesOptions = {
...
export default function MassageLabWaveCurrentBackground({
  ...
  const backgroundColor = massageLabWaveCurrent?.backgroundColor
  const waveColor1 = massageLabWaveCurrent?.waveColor1
  const waveColor2 = massageLabWaveCurrent?.waveColor2
  const waveColor3 = massageLabWaveCurrent?.waveColor3
  const waveSpeedX = massageLabWaveCurrent?.waveSpeedX
  const waveSpeedY = massageLabWaveCurrent?.waveSpeedY
  const waveAmpX = massageLabWaveCurrent?.waveAmpX
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabWaveCurrent, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabWaveCurrentCanvas} />
    </div>
  )
}
...
function resolveWavesOptions(options: MassageLabWaveCurrentOptions): ResolvedWavesOptions {
  return {
    backgroundColor: normalizeHexColor(options.backgroundColor, DEFAULT_MASSAGE_LAB_WAVES.backgroundColor),
    waveColor1: normalizeHexColor(options.waveColor1, DEFAULT_MASSAGE_LAB_WAVES.waveColor1),
    waveColor2: normalizeHexColor(options.waveColor2, DEFAULT_MASSAGE_LAB_WAVES.waveColor2),
    waveColor3: normalizeHexColor(options.waveColor3, DEFAULT_MASSAGE_LAB_WAVES.waveColor3),
    waveSpeedX: clampNumber(options.waveSpeedX, DEFAULT_MASSAGE_LAB_WAVES.waveSpeedX, 0.001, 0.1),
    waveSpeedY: clampNumber(options.waveSpeedY, DEFAULT_MASSAGE_LAB_WAVES.waveSpeedY, 0.001, 0.1),
    waveAmpX: clampNumber(options.waveAmpX, DEFAULT_MASSAGE_LAB_WAVES.waveAmpX, 8, 64),
  }
}
