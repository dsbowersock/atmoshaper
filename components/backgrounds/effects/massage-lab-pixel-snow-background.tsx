...
import type { BackgroundEffectProps, MassageLabPixelSnowOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type PixelSnowVariant = NonNullable<MassageLabPixelSnowOptions["variant"]>
type ResolvedPixelSnowOptions = Required<MassageLabPixelSnowOptions>
...
const DEFAULT_MASSAGELAB_PIXEL_SNOW: ResolvedPixelSnowOptions = {
  color: "#FFFFFF",
  flakeSize: 0.01,
  minFlakeSize: 1.25,
  pixelResolution: 200,
...
export default function MassageLabPixelSnowBackground({
  ...
  const options = useMemo(
    () => resolvePixelSnowOptions(massageLabPixelSnow),
    [massageLabPixelSnow],
  )
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.massageLabPixelSnowCanvas, className)}
    />
  )
}
...
function resolvePixelSnowOptions(options: MassageLabPixelSnowOptions | undefined): ResolvedPixelSnowOptions {
  return {
    color: resolveHexColor(options?.color, DEFAULT_MASSAGELAB_PIXEL_SNOW.color),
    flakeSize: resolveNumber(options?.flakeSize, DEFAULT_MASSAGELAB_PIXEL_SNOW.flakeSize, 0.001, 0.08),
    minFlakeSize: resolveNumber(
      options?.minFlakeSize,
      DEFAULT_MASSAGELAB_PIXEL_SNOW.minFlakeSize,
      0.1,
      6,
    ),
    pixelResolution: resolveNumber(
      options?.pixelResolution,
      DEFAULT_MASSAGELAB_PIXEL_SNOW.pixelResolution,
      40,
      640,
    ),
    speed: resolveNumber(options?.speed, DEFAULT_MASSAGELAB_PIXEL_SNOW.speed, 0, 5),
    depthFade: resolveNumber(options?.depthFade, DEFAULT_MASSAGELAB_PIXEL_SNOW.depthFade, 1, 40),
    farPlane: resolveNumber(options?.farPlane, DEFAULT_MASSAGELAB_PIXEL_SNOW.farPlane, 4, 80),
    brightness: resolveNumber(options?.brightness, DEFAULT_MASSAGELAB_PIXEL_SNOW.brightness, 0.1, 4),
    gamma: resolveNumber(options?.gamma, DEFAULT_MASSAGELAB_PIXEL_SNOW.gamma, 0.1, 2),
    density: resolveNumber(options?.density, DEFAULT_MASSAGELAB_PIXEL_SNOW.density, 0.02, 1),
    variant: resolveVariant(options?.variant),
    direction: resolveNumber(options?.direction, DEFAULT_MASSAGELAB_PIXEL_SNOW.direction, 0, 360),
  }
}
...
function resolveVariant(value: PixelSnowVariant | undefined): PixelSnowVariant {
  ...
  return DEFAULT_MASSAGELAB_PIXEL_SNOW.variant
}
...
function parseHexColorToRgb(value: string): RgbColor {
  const color = resolveHexColor(value, DEFAULT_MASSAGELAB_PIXEL_SNOW.color).slice(1)
