...
import type {
  ...
  MassageLabLightPillarOptions,
} from "./css-backgrounds"

type LightPillarQuality = NonNullable<MassageLabLightPillarOptions["quality"]>
type LightPillarBlendMode = NonNullable<MassageLabLightPillarOptions["mixBlendMode"]>
type ResolvedLightPillarOptions = Required<MassageLabLightPillarOptions>
...
const DEFAULT_MASSAGELAB_LIGHT_PILLAR: ResolvedLightPillarOptions = {
  topColor: "#5227FF",
  bottomColor: "#FF9FFC",
  intensity: 1,
  rotationSpeed: 0.3,
...
export default function MassageLabLightPillarBackground({
  ...
  const topColor = massageLabLightPillar?.topColor
  const bottomColor = massageLabLightPillar?.bottomColor
  const intensity = massageLabLightPillar?.intensity
  const rotationSpeed = massageLabLightPillar?.rotationSpeed
  const interactive = massageLabLightPillar?.interactive
  const glowAmount = massageLabLightPillar?.glowAmount
  const pillarWidth = massageLabLightPillar?.pillarWidth
  const pillarHeight = massageLabLightPillar?.pillarHeight
  const noiseIntensity = massageLabLightPillar?.noiseIntensity
  const mixBlendMode = massageLabLightPillar?.mixBlendMode
  const pillarRotation = massageLabLightPillar?.pillarRotation
  const quality = massageLabLightPillar?.quality
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.massageLabLightPillar, className)}
      style={style}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.massageLabLightPillarCanvas} />
    </div>
  )
}
...
function resolveLightPillarOptions(
  options: MassageLabLightPillarOptions | undefined,
): ResolvedLightPillarOptions {
  return {
    topColor: normalizeHexColor(options?.topColor, DEFAULT_MASSAGELAB_LIGHT_PILLAR.topColor),
    bottomColor: normalizeHexColor(options?.bottomColor, DEFAULT_MASSAGELAB_LIGHT_PILLAR.bottomColor),
    intensity: clampNumber(options?.intensity, DEFAULT_MASSAGELAB_LIGHT_PILLAR.intensity, 0.1, 3),
    rotationSpeed: clampNumber(options?.rotationSpeed, DEFAULT_MASSAGELAB_LIGHT_PILLAR.rotationSpeed, 0, 2),
    interactive: options?.interactive === true,
    glowAmount: clampNumber(options?.glowAmount, DEFAULT_MASSAGELAB_LIGHT_PILLAR.glowAmount, 0.001, 0.03),
    pillarWidth: clampNumber(options?.pillarWidth, DEFAULT_MASSAGELAB_LIGHT_PILLAR.pillarWidth, 0.5, 8),
    pillarHeight: clampNumber(options?.pillarHeight, DEFAULT_MASSAGELAB_LIGHT_PILLAR.pillarHeight, 0.1, 2),
    noiseIntensity: clampNumber(options?.noiseIntensity, DEFAULT_MASSAGELAB_LIGHT_PILLAR.noiseIntensity, 0, 1),
    mixBlendMode: resolveBlendMode(options?.mixBlendMode),
    pillarRotation: clampNumber(options?.pillarRotation, DEFAULT_MASSAGELAB_LIGHT_PILLAR.pillarRotation, -180, 180),
...
function resolveQuality(value: LightPillarQuality | undefined): LightPillarQuality {
  return value === "low" || value === "medium" || value === "high"
    ? value
    : DEFAULT_MASSAGELAB_LIGHT_PILLAR.quality
}
...
function resolveBlendMode(value: LightPillarBlendMode | undefined): LightPillarBlendMode {
  return value === "normal" || value === "lighten" || value === "plus-lighter" || value === "screen"
    ? value
    : DEFAULT_MASSAGELAB_LIGHT_PILLAR.mixBlendMode
}
