...
import type { BackgroundEffectProps, MassageLabBalatroOptions } from "./css-backgrounds"

type ResolvedBalatroOptions = Required<MassageLabBalatroOptions>
...
const DEFAULT_MASSAGELAB_BALATRO: ResolvedBalatroOptions = {
  spinRotation: -2,
  spinSpeed: 7,
  offsetX: 0,
  offsetY: 0,
...
export default function MassageLabBalatroBackground({
  ...
  const balatroSpinRotation = massageLabBalatro?.spinRotation
  const balatroSpinSpeed = massageLabBalatro?.spinSpeed
  const balatroOffsetX = massageLabBalatro?.offsetX
  const balatroOffsetY = massageLabBalatro?.offsetY
  const balatroColor1 = massageLabBalatro?.color1
  const balatroColor2 = massageLabBalatro?.color2
  const balatroColor3 = massageLabBalatro?.color3
  const balatroContrast = massageLabBalatro?.contrast
  const balatroLighting = massageLabBalatro?.lighting
  const balatroSpinAmount = massageLabBalatro?.spinAmount
  const balatroPixelFilter = massageLabBalatro?.pixelFilter
  const balatroSpinEase = massageLabBalatro?.spinEase
  const balatroIsRotate = massageLabBalatro?.isRotate
  const balatroMouseInteraction = massageLabBalatro?.mouseInteraction
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.massageLabBalatroCanvas, className)}
    />
  )
}
...
function resolveBalatroOptions(options?: MassageLabBalatroOptions): ResolvedBalatroOptions {
  return {
    spinRotation: resolveNumber(options?.spinRotation, DEFAULT_MASSAGELAB_BALATRO.spinRotation, -8, 8),
    spinSpeed: resolveNumber(options?.spinSpeed, DEFAULT_MASSAGELAB_BALATRO.spinSpeed, 0, 14),
    offsetX: resolveNumber(options?.offsetX, DEFAULT_MASSAGELAB_BALATRO.offsetX, -1, 1),
    offsetY: resolveNumber(options?.offsetY, DEFAULT_MASSAGELAB_BALATRO.offsetY, -1, 1),
    color1: resolveHex(options?.color1, DEFAULT_MASSAGELAB_BALATRO.color1),
    color2: resolveHex(options?.color2, DEFAULT_MASSAGELAB_BALATRO.color2),
    color3: resolveHex(options?.color3, DEFAULT_MASSAGELAB_BALATRO.color3),
    contrast: resolveNumber(options?.contrast, DEFAULT_MASSAGELAB_BALATRO.contrast, 0.5, 8),
    lighting: resolveNumber(options?.lighting, DEFAULT_MASSAGELAB_BALATRO.lighting, 0, 1),
    spinAmount: resolveNumber(options?.spinAmount, DEFAULT_MASSAGELAB_BALATRO.spinAmount, 0, 1),
    pixelFilter: resolveNumber(options?.pixelFilter, DEFAULT_MASSAGELAB_BALATRO.pixelFilter, 120, 1200),
    spinEase: resolveNumber(options?.spinEase, DEFAULT_MASSAGELAB_BALATRO.spinEase, 0, 3),
    isRotate: options?.isRotate ?? DEFAULT_MASSAGELAB_BALATRO.isRotate,
    mouseInteraction: options?.mouseInteraction ?? DEFAULT_MASSAGELAB_BALATRO.mouseInteraction,
  }
}
