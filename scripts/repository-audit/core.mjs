...
export function verifyLegacyReferenceBaseline(actual, baseline, policy) {
  ...
  const additions = actual
    ...
    .map((entry) => toBaselineEntry(entry, classifyCandidate(entry, policy)))
  const unclassified = additions
    .filter((entry) => (
      entry.category === "pre-rebrand-public-copy" || entry.category === "unclassified"
    ))
