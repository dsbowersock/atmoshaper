...
Phase 4 [PR #3](https://github.com/dsbowersock/atmoshaper/pull/3) and Phase 5 [PR #4](https://github.com/dsbowersock/atmoshaper/pull/4) are merged. Phase 6 is being delivered as smaller dependency-ordered replacements for preserved [PR #5](https://github.com/dsbowersock/atmoshaper/pull/5). Replacement PRs #6–#12 remain open and unmerged after completing their individual exact-head review and CI gates. [PR #13](https://github.com/dsbowersock/atmoshaper/pull/13) is the active Task 8 legal-identity replacement. Successive hosted and local review rounds exposed verified server-session, public-session and deterministic-clock boundary findings; the reviewed product amendment was published as `2ab6382d2e944b75e8eb0e76808870f205d21736` and its durable record synchronization as `ecf30584cf689edca4bfe2566e1d9a795253fd29`. Strict CI on that synchronized head then exposed a Browser-QA fixture gap at the new legal-acceptance boundary. The bounded repair was published as `26c76ada77c7d3977af666942101b6e5018e15f3`: it creates current registration acceptances for authorized synthetic database users, removes those restricted rows and the exact owned user in one rollback-safe transaction, and supplies provider-free client-session/account-bootstrap ownership without weakening connected-mode assertions. Its strict CI passed, but CodeRabbit identified two valid fixture-contract follow-ups: provider-free Auth.js sessions require an expiry and standalone same-origin HTML without the shared account bootstrap must pass through unchanged. Local review then tightened expiry bounds and origin confinement and removed a duplicate session-lifetime owner, so the signed cookie and provider-free projection now consume one canonical one-hour constant. Its executable oracle also locks the real runtime import and uses fixed-clock probes across multiple lifetimes to reject pointwise caps and arithmetic offsets. Those three owned paths are locally validated but not yet published. GitHub and the ignored review handoff are authoritative for the resulting current head, hosted-review and strict-CI status; exact-head coverage from both reviewers and every required CI job remains mandatory before any merge decision.
