...
Phase 4 [PR #3](https://github.com/dsbowersock/atmoshaper/pull/3) and Phase 5 [PR #4](https://github.com/dsbowersock/atmoshaper/pull/4) are merged. Phase 6 is being delivered as smaller dependency-ordered replacements for preserved [PR #5](https://github.com/dsbowersock/atmoshaper/pull/5). Replacement PRs #6–#12 remain open and unmerged after completing their individual exact-head review and CI gates. [PR #13](https://github.com/dsbowersock/atmoshaper/pull/13) is the active Task 8 legal-identity replacement. Successive hosted and local review rounds exposed verified server-session, public-session, Browser-QA fixture and deterministic-clock boundary findings. The fixture/oracle hardening passed independent SPEC then QUALITY and was published as `fd7d6124b57177a5f2266af8af82fb0561aba332`. Both hosted reviewers completed exact-head review and each found one valid follow-up: matching-email Google linking could strand a proven user at the current legal-acceptance gate, while a real-clock assertion could falsely fail if the wall clock moved backward. Both findings received strict RED/GREEN repairs in the 43-path candidate. Frozen v18 passed SPEC, but QUALITY proved its positive-only legal-routing test admitted an `&&` to `||` mutation. Frozen v19 then passed SPEC, but QUALITY proved its two mismatched pairs admitted prefix matching and string coercion. Frozen v20 passed SPEC, but QUALITY proved the corpus still omitted missing code, a Unicode lookalike and rejected JSON. The test-only recurrence matrix covers those cases plus nonnumeric status, near-match, case, whitespace, null, array and object values and rejects the `||`, prefix, coercion, undefined-acceptance, Unicode-normalization and JSON-fallback mutations without changing production. The full Node suite passed 4,755 total with 4,752 passes, three host-dependent skips and zero failures. Superseded strict CI `35668571160` passed code quality, build and every browser test body (lane 3: 152 passed, 42 skipped), but lane 3 and aggregate failed because the GitHub artifact intermediary returned 403 while uploading diagnostics. The repaired 43-path v21 package passed independent SPEC then QUALITY and was published as `22eb0ef690b354317a74a74bf0318aa6c6f2a733`. Both hosted reviewers completed exact-head review of that head, and all seven jobs in strict CI `35682219253` passed without PNG changes. PR #13 remains open and unmerged. The post-v21 OAuth amendment `7ad60de8519e4a1d4bcc0bd892583b0c54d80186` makes public Google callbacks use the acceptance-filtered session identity; at the 2026-09-22 checkpoint, PR #13 completed both hosted reviews and strict CI on its published head. Task 9 is underway only in the isolated local worktree; combined-source equivalence, Task 9 reviews and CI, and the separate Linux Atmosphere snapshot decision remain pending. GitHub and the ignored review handoff are authoritative for live status. No merge is authorized.
