...
const JURISDICTION_OVERRIDES = Object.freeze({
  OH: {
    ...
    notes: `Ohio eLicense is the primary public lookup source and ${PUBLIC_PRODUCT_IDENTITY.name} can run an automatic verifier for Ohio massage therapy licenses.`,
  },
})
...
export function findMassageJurisdiction(value) {
  ...
  return {
    ...
    notes: override?.notes ?? `No automated ${PUBLIC_PRODUCT_IDENTITY.name} verifier is available yet; use pending/manual review.`,
  }
}
...
export function getJurisdictionVerificationPlan(value) {
  ...
  if (!jurisdiction) {
    return {
      ...
      sourceUrl: null,
      message: `${PUBLIC_PRODUCT_IDENTITY.name} does not recognize that jurisdiction yet.`,
    }
  }
    ...
    message:
      jurisdiction.supportStatus === "ADAPTER_AVAILABLE"
        ...
        : jurisdiction.lookupUrl
          ? `Public license lookup is available, but ${PUBLIC_PRODUCT_IDENTITY.name} will keep the role pending until verification is completed.`
