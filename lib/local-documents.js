...
export function createEditableDocumentHtml({ title, body, generatedAt = new Date() }) {
  ...
  return [
    ...
    `<div class="notice">Local-first export. ${PUBLIC_PRODUCT_IDENTITY.name} did not upload this clinical document. The user controls storage and sharing.</div>`,
...
export function mergeLocalDocumentData(defaults, imported, { discriminatorKey, discriminatorValue }) {
  if (!isPlainObject(imported)) {
    throw new LocalDocumentError(`Expected an ${PUBLIC_PRODUCT_IDENTITY.name} JSON object.`)
  }
