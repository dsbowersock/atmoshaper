...
export function createFormResponseExportText({ response, template, client }) {
  ...
  if (!normalizedTemplate || !normalizedResponse) {
    return `${PUBLIC_PRODUCT_IDENTITY.name} Local Intake Export\nCould not read this form response.`
  }

  const lines = [
    `${PUBLIC_PRODUCT_IDENTITY.name} Local Intake Export`,
    `Local-first export. ${PUBLIC_PRODUCT_IDENTITY.name} did not upload this clinical document.`,
    "",
...
export function createWorkspaceExport(workspace, exportedAt = new Date()) {
  ...
  return {
    ...
    localFirst: true,
    notice: `${PUBLIC_PRODUCT_IDENTITY.name} did not upload these local clinical documents. User controls storage and sharing.`,
  }
}
...
export async function decryptEncryptedIntakeBundle(bundle, password) {
  if (!isPlainObject(bundle) || bundle.format !== "massagelab-local-encrypted-bundle") {
    throw new Error(`Expected an ${PUBLIC_PRODUCT_IDENTITY.name} encrypted bundle.`)
  }
...
export async function decryptEncryptedIntakeWorkspaceVault(vault, password) {
  if (!isEncryptedIntakeWorkspaceVault(vault)) {
    throw new Error(`Expected an ${PUBLIC_PRODUCT_IDENTITY.name} intake workspace vault.`)
  }
