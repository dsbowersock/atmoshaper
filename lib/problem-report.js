...
/**
 * Provider-facing event identity retained for stable Sentry grouping. Public
 * presentation changes do not retire durable operational identifiers.
 */
const PROBLEM_REPORT_SENTRY_MESSAGE = "MassageLab privacy-safe problem report"
...
export const PROBLEM_REPORT_REQUEST_TIMEOUT_MS = 10_000

/**
 * Keeps the polite retry announcement stable while the visible countdown ticks,
 * then changes it exactly once when manual retry becomes available.
 *
 * @param {number} retrySeconds
 */
export function problemReportRetryAnnouncement(retrySeconds) {
  return Number.isFinite(retrySeconds) && retrySeconds > 0
    ? "Diagnostic reports are temporarily paused. Wait before trying again. This page will not resend the report automatically."
...
export function buildProblemReportSentryPayload(input = {}) {
  ...
  return {
    message: PROBLEM_REPORT_SENTRY_MESSAGE,
    level: "warning",
    tags: {
