...
export const MEMBERSHIP_PLAN_DETAILS = Object.freeze({
  SUPPORTER: {
    name: `${PUBLIC_PRODUCT_IDENTITY.name} Supporter Membership`,
    eyebrow: "Alpha support",
    description: `For users who want every premium background, saved Chimer colors, and Supporter status while helping keep ${PUBLIC_PRODUCT_IDENTITY.name} development moving without ads or data sales.`,
    currentFeatures: [
...
async function buildMembershipPricingCatalog({ env = process.env, stripeClient } = {}) {
  ...
  // Imported nested collections are copied before deep-freeze: currentFeatures,
  // roadmapNotes, and interval objects get fresh containers; amount choices
  // contain only primitive fields and are spread into fresh objects below.
