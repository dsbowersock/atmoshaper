...
export function mergeProfessionalRecordVaultPayloadChanges(basePayload, nextPayload, currentPayload) {
  const base = normalizeProfessionalRecordVaultPayload(basePayload)
  const next = normalizeProfessionalRecordVaultPayload(nextPayload)
  const current = normalizeProfessionalRecordVaultPayload(currentPayload)
  const records = { ...current.records }

  for (const recordType of ["soap", "intake", "journal", "rom"]) {
    if (stableJson(base.records[recordType]) !== stableJson(next.records[recordType])) {
      records[recordType] = next.records[recordType]
    }
  }

  return normalizeProfessionalRecordVaultPayload({
    schemaVersion: 1,
    records,
  })
}
...
export async function decryptEncryptedProfessionalRecordVault(vault, password) {
  if (!isEncryptedProfessionalRecordVault(vault)) {
    throw new ProfessionalRecordVaultError(`Expected an ${PUBLIC_PRODUCT_IDENTITY.name} professional-record vault.`)
  }
...
export async function decryptEncryptedProfessionalRecordBundle(bundle, password) {
  if (!isEncryptedProfessionalRecordVaultBundle(bundle)) {
    throw new ProfessionalRecordVaultError(`Expected an ${PUBLIC_PRODUCT_IDENTITY.name} encrypted professional-record bundle.`)
  }
