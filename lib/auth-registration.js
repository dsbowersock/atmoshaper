...
export const REGISTRATION_VERIFICATION_FAILED_MESSAGE =
  `Your account was created, but ${PUBLIC_PRODUCT_IDENTITY.name} could not send the verification email. Please try again later or contact support.`
...
/**
 * Builds the verification-resend page path without including an email address.
 *
 * @param {unknown} callbackUrl
 * @returns {string}
 */
export function buildVerificationRequestPath(callbackUrl) {
  const params = new URLSearchParams({
    callbackUrl: safePostLegalAcceptanceCallback(callbackUrl),
  })
...
/**
 * Normalizes verification-page query values before token lookup and sign-in
 * navigation.
 *
 * @param {{ token?: unknown, callbackUrl?: unknown } | null | undefined} params
 * @returns {{ token: string, callbackUrl: string }}
 */
export function normalizeEmailVerificationParams(params) {
  return {
    token: typeof params?.token === "string" ? params.token : "",
    callbackUrl: safePostLegalAcceptanceCallback(
...
/**
 * Delivers a registration verification message with an app-local callback.
 *
 * @template T
 * @param {(email: string, token: string, callbackUrl: string) => T} sender
 * @param {string} email
 * @param {string} token
 * @param {unknown} callbackUrl
 * @returns {T}
 */
export function sendRegistrationVerification(sender, email, token, callbackUrl) {
  return sender(email, token, safePostLegalAcceptanceCallback(callbackUrl))
}
