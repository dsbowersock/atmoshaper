...
const atmosphereStations = Object.freeze([
  {
    id: "mlab-proof-drone",
    title: `${PUBLIC_PRODUCT_IDENTITY.name} Proof Drone`,
    artist: PUBLIC_PRODUCT_IDENTITY.name,
    sourceType: "tone-generator",
    enabled: true,
    description:
      "A soft, steady drone with gentle beating tones for a simple calm-room bed.",
    tags: ["massage room", "soft drone", "calm"],
    attribution: {
      artist: "MassageLab",
...
export function getAtmosphereStationById(stationId) {
  ...
  if (!station) {
    throw new Error(`Unknown ${ATMOSPHERE_PUBLIC_LABELS.name} station: ${stationId}`)
  }
