  ...
  function setMetadata(metadata) {
    ...
    try {
      ...
      const init = {
        ...
        artist: metadata.artist || PUBLIC_PRODUCT_IDENTITY.name,
        album: `${PUBLIC_PRODUCT_IDENTITY.name} ${ATMOSPHERE_PUBLIC_LABELS.name}`,
      }
      if (metadata.id?.startsWith("atmoshaper:")) {
        init.artwork = [
          {
      ...
      mediaSession.metadata = createMetadata(init)
    } catch {
