...
MassageLab hosts the audio runtime in the app. It does not embed Generative.fm as a remote player UI. `/music` now exposes the locally adapted Observable Streams station plus the full 56-piece Alex Bainter Generative.fm package catalog through the global music provider, route-persistent playback, grouped station browsing, Atmosphere-only swipe/scroll station rails, deterministic organic-geometric SVG station artwork, and the placement-aware audio toolbar opposite the selected app bar edge. All 57 playable stations—Observable Streams plus the 56 package pieces—are currently served from hosted public-media sample indexes. `/wellness/breathing` carries the first public Calmness-style breathing guide as a separate Wellness tool that does not store account data or clinical records. The hosted Generative.fm runtime prepares sample-index metadata and browser modules for eight starter stations during idle. Three of those stations and station hover can also warm compressed sample payloads when data saving is off and the connection is not slow-2g or 2g; browsers without connection information permit that warmup. Focus and play-button pointer-down remain metadata-only. Provider sample fetch/decode work is batched so large stations do not request every sample in one unbounded burst. Actual Tone start, Transport start, output nodes, and WAV fallback payload loading remain user-gesture gated. The audio toolbar shows loading progress while a station prepares and offers previous/next station controls; supported browsers also receive Media Session metadata and play/pause/stop/previous/next handlers for notification and lock-screen controls. Generative.fm handoffs fade the outgoing station down and fade the incoming station up to soften abrupt first notes. All 57 playable stations have Ogg Opus, AAC/M4A, and MP3 sidecar indexes for broader browser coverage, and keep their original WAV indexes as the final fallback.
...
The June 18–19, 2026 execution plans remain in the immutable MassageLab source. Their upload counts, licensing/source mappings, HTTP checks, old routes, and performance recipes are dated evidence, not a current operations checklist or a new provider verification. Current behavior is owned by this page and the linked implementation; every omission and restoration blob is recorded in the [cleanup register](../rebrand/atmoshaper-cleanup-register.md#phase-3-documentation-omissions--2026-09-09).

Coverage and hosting: [First hosting batch](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-18-atmosphere-first-batch-hosting.md); [Sample coverage](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-18-atmosphere-generative-fm-sample-coverage.md); [Second hosting batch](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-18-atmosphere-second-batch-hosting.md); [Third hosting batch and listener copy](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-18-atmosphere-third-batch-listener-copy.md); [Remaining generators](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-19-atmosphere-remaining-generators.md); [Rendered piano and source rollout](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-19-atmosphere-rendered-piano-batch.md).

Formats and startup: [Hosted Opus sidecars](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-18-atmosphere-hosted-opus-sidecars.md); [Startup performance](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-18-atmosphere-startup-performance.md); [Web-audio format pilot](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-18-atmosphere-web-audio-format-pilot.md); [AAC/MP3 sidecars](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-19-atmosphere-aac-mp3-sidecars.md); [Playback performance](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-19-atmosphere-playback-performance.md).

CI history: [CI cache](https://github.com/dsbowersock/massagelab/blob/e74045c2fc85c2cb4df176fdb1aff2137c4d9848/docs/superpowers/plans/2026-06-19-ci-build-cache.md); [current CI owner](ci-pr-checks.md).

## Public R2 Sample Hosting

The June 2026 upload and HTTP results below are retained historical receipts. They have not been rechecked during Phase 3. The command examples describe existing tooling; any actual upload or provider change requires separate authorization under the [migration charter](../rebrand/atmoshaper-migration-charter.md).
...
Also on 2026-06-19, the AAC/MP3 sidecar rollout expanded compressed fallback coverage to all 57 playable stations: Observable Streams plus the 56 Alex Bainter package pieces.
...
- `/music` exposes Observable Streams plus the full 56-piece Alex Bainter Generative.fm package catalog through MassageLab's global music provider, swipe/scroll category rails, deterministic organic-geometric SVG station artwork, and a persistent placement-aware audio toolbar. `/atmosphere` and `/wellness/atmosphere` are not retained after the navigation move, and `/browse` remains available as a compatibility grid/workbench for the same station runtime.
...
- Startup prewarm validates hosted sample-index metadata and imports browser runtime modules for eight starter stations after idle. The workspace also requests compressed payload warmup for Observable Streams, aisatsana, and Day/Dream, and station hover can request it for the hovered station. `canPrewarmCompressedSamplePayloads()` disables that work for `saveData`, `slow-2g`, and `2g`; absent connection information permits it. Focus, play-button pointer-down, and carousel centering pass no payload option and remain metadata-only. All prewarm paths avoid starting Tone, Transport, and output nodes; payload warmup skips WAV. Current owners are [`app/browse/workspace.tsx`](../../app/browse/workspace.tsx), [`station-carousel-card.tsx`](../../components/atmosphere/station-carousel-card.tsx), [`music-provider.tsx`](../../components/providers/music-provider.tsx), and [`generative-fm-runtime.ts`](../../lib/atmosphere/generative-fm-runtime.ts).
...
[CI and PR checks](ci-pr-checks.md) and [the workflow](../../.github/workflows/ci.yml) own current build/cache operations. Browser build restores `.next/cache` through pinned `actions/cache@v6.1.0`, runs `build:browser-qa`, and shares an attempt-qualified runtime artifact with four browser lanes. The June 2026 `build:next` recipe and v4 cache pin are historical; use the current owners for changes and validation.
