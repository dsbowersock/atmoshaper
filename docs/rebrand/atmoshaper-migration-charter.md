...
- Current owners are the [architecture map](../architecture.md), [decisions](../decisions/README.md), and [account-security wiki](../wiki/account-security.md). ADR 0001 is Accepted; ADR 0002 records the implemented Phase 6 identity/legal decision but remains Proposed pending final Task 9 integration, while ADRs 0003–0004 remain Proposed.
...
- The Phase 6 preview presentation and versioned legal transition are implemented in the recovery source and distributed across smaller replacement PRs. Open, unmerged PR #13 has clean exact-head reviews and all seven strict CI jobs green at `5345c7cc2f413e4dfd33def15bff5c8e99151243`; final Task 9 is an isolated local candidate, not a published or reviewed PR. The source's `75e2741aa3ea5d2cb24fef62fdefb1b5564d4bfe` systemic public-audio-label repair remains part of equivalence proof. The current full-text, approved temporary square-mark, and hidden brand-link states, station title `Drone`, artist `AtmoShaper`, stable ID `mlab-proof-drone`, unchanged artwork bytes/revision, and current legal v3 documents with immutable v2 archives preserve their separate owners. Final source equivalence and the Linux Atmosphere snapshot decision remain pending. Old-origin recovery, provider staging, deployment, DNS/domain changes, production/database/payment/email/media actions and merge remain future gates.
...
- **Legal/operator separation:** Phase 6 separately changed current legal presentation and the proprietary license to `Derrick Bowersock, doing business as AtmoShaper` after exact prior-version archiving and under new current version IDs. Existing acceptance rows and archived text remain immutable. No trademark, registration or entity-status claim is permitted.
