...
| Public product identity | [Phase 6 design](superpowers/specs/2026-09-14-atmoshaper-phase6-preview-rebrand-design.md), [ADR 0002](decisions/0002-public-identity-legal-and-compatibility-boundaries.md) | [Public identity](../lib/public-product-identity.js) owns the product name, short name, and approved presentation-asset availability. Consumers own contextual sentences; legal, provider, domain, persistence, and compatibility identifiers remain separate. |
| Shared audio | [Atmosphere audio](wiki/atmosphere-audio.md) | [Music provider](../components/providers/music-provider.tsx) owns one global playback surface shared by stations and the live-session mixer at [Music](../app/music/). The planned [public feature labels](../lib/atmosphere/public-labels.js) own only `Atmosphere` nouns. [Atmosphere runtime](../lib/atmosphere/), [internal mixer domain](../lib/atmoshaper/) and [production catalog](../data/atmoshaper/production-audio-catalog.json) retain source, release and compatibility ownership. The live recipe is memory-only; saved mixes remain future work. |
| Legal presentation and acceptance | [Phase 6 design](superpowers/specs/2026-09-14-atmoshaper-phase6-preview-rebrand-design.md), [ADR 0002](decisions/0002-public-identity-legal-and-compatibility-boundaries.md) | [Legal documents](../lib/legal-documents.js) remain the only current registry and [legal acceptance](../lib/legal-acceptance.js) retains versioned row ownership. Phase 6 adds evidence-only immutable prior-version archives before publishing new current versions; archives never become a runtime fallback. |
...
The central public-brand owner is implemented by merged Phase 5. Phase 6's
preview rebrand, noun-only Atmosphere owner, and versioned legal transition are
approved and planned but not yet implemented. Dedicated old-origin recovery and
parallel provider staging remain future work.
...
presentation only after exact prior-version archiving and under new version IDs;
it never rewrites old acceptance rows, compatibility IDs, or hosted state.
