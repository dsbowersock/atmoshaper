...
**Architecture:** Treat the latest verified MassageLab `main` commit as an immutable source tree. Phase 1 preparation was reviewed on `codex/atmoshaper-migration-preflight` and merged by PR #206; Phase 2 exports exact merged `main` `e74045c2fc85c2cb4df176fdb1aff2137c4d9848` without `.git`, overlays only the ten reviewed relock documents from `codex/atmoshaper-phase2-relock`, initializes fresh Git history in a sibling repository, verifies parity locally, and then publishes `main` plus a review branch without merging it. Historical material stays in `dsbowersock/massagelab`; the new repository carries exact lineage and only evidence-backed omissions.
...
## 2026-09-08 Phase 2 source relock amendment

PR #206 merged reviewed head `971c453ebb50127bb3ffd1d9f5e4f133dbf9df82` as `main` merge `e74045c2fc85c2cb4df176fdb1aff2137c4d9848`; both resolve to tree `0370e1d2749f9644efdc3ccbcb832c8d7965ec0d`. Exact merge-head Code quality, Browser build, Browser QA lanes 1–4, aggregate `qa`, and CodeQL Actions/JavaScript/Python passed, and Vercel reported its automatic `main` deployment complete. This amendment supersedes every forward-looking use of `fa78ca01a42179329cc223df77c76f308e76320b` or the old preparation branch as the Task 5 source/overlay owner; those values remain below only as historical Phase 1 evidence.

The immutable Task 5 source is `e74045c2fc85c2cb4df176fdb1aff2137c4d9848`. Its 1,905 paths already contain the reviewed parity tooling, snapshots, and source-QA corrections. The current difference contract is four omissions, 15 replacements, one Task 5 addition, six Task 6 additions, and ten overlays: `1,905 - 4 + 1 = 1,902` Task 5 paths and `1,902 + 6 = 1,908` final paths. Task 5 is the next gated action after Task 4R review and coordinator commit.
...
- Baseline Lock: immediately before Task 5, manifest `sourceCommit`, tracked `origin/main`, and live GitHub `main` must agree, and the relock branch must descend from that source. The local `main` pointer is historical-only and is not export authority.
...
- Drift Rule: if tracked/live `main` advances before export or the relock branch lacks ancestry from the manifest source, discard stale source measurements and restart source selection against the new clean head. Do not merge or rebase runtime changes into the locked snapshot.
...
**Historical completed Phase 1 procedure:** The local-branch equality steps retained below record the original 2026-09-06 Task 1 lock only. They are not operative Task 5 source-selection rules; the Task 4R amendment and Task 5 Step 1 govern the current manifest/tracked/live agreement and relock-ancestry checks.
...
**Historical Task 1 equality check only:** compare the printed 40-character SHA with both `git rev-parse main` and `git rev-parse origin/main`. At the original Phase 1 lock, execution stopped if any differed. This retained receipt is not an operative Task 5 rule; use the Task 4R manifest/tracked/live/ancestry gate instead.
...
**Historical completed Phase 1 procedure:** Any local-`main` source assignment retained below documents the original inventory run only. Current execution reads `sourceCommit` from the reviewed manifest and applies the Task 5 Step 1 tracked/live agreement and relock-ancestry gate.
...
```powershell
$sourceSha = git rev-parse main # Historical Task 2 inventory source only; never Task 5 export authority.
...
The SHA shown is the 2026-09-08 relocked exact merged `main`. Immediately before Task 5, require the manifest `sourceCommit`, tracked `origin/main`, and live GitHub `main` to agree, and require the relock branch to descend from that source; any disagreement returns to source selection. The local `main` pointer is retained historical state and is not export authority. Populate every array with exact normalized repository file paths. `task5AddPaths` and `task6AddPaths` divide source-absent files by their creating task. Expand every omitted directory candidate into exact tracked file paths in `omitPaths`; the execution step never infers descendants. `overlayPaths` contains only files copied from `codex/atmoshaper-phase2-relock` whose reviewed branch bytes intentionally differ from the locked source or that the export must otherwise carry. `replacePaths` identifies every source-present file whose destination contents intentionally change: four fresh destination authority documents, non-overlay `package.json` for Task 6 audit-script registration, and all ten overlays. Sort every array by code-point order, reject duplicates within an array, and require each source-present overlay to appear in `replacePaths`. The JSON and Markdown difference contract must describe the same path set; a mismatch stops execution.

The historical pre-merge contract was one omission, 44 replacements, 47 Task 5 additions, six Task 6 additions, and 86 overlays. PR #206 incorporated 46 of those additions and every reviewed source-byte correction into `main`, and also added three MassageLab-only Aegis execution records. Reclassification against the merged tree plus the five carried-document historical-header corrections gives four omissions, 15 replacements, one Task 5 addition, six Task 6 additions, and ten overlays. The destination totals remain `1,905 - 4 + 1 = 1,902` after Task 5 and `1,902 + 6 = 1,908` after Task 6.
...
**Verification:** Source capture passes on desktop/mobile; signed-in tests execute rather than skip; the 63 migration-harness checks and 34 account/form regression checks pass; existing CI-lane manifest and its exact 17 specs/34 assignments remain unchanged; the relocked manifest and embedded inventory JSON match with four omissions, 15 replacements, one Task 5 addition, six Task 6 additions, ten overlays, and exact 1,902/1,908 path totals; no browser mutation request or persistent fixture remains. Record read-only external requests separately. Exact migration invocations use one worker, refuse an occupied server port and use a fresh current-checkout, telemetry-inert Browser-QA build. Ordinary runs preserve their existing worker default and CI remains one worker; do not override migration runs back to concurrent workers.
...
Historical Task 3 expectation: 63/63 migration-harness checks plus 34/34 account/form checks (97 total), successful TypeScript/lint, and no lane-manifest delta. The relock must separately verify the manifest against its embedded JSON, sorted unique normalized path arrays, source-present omit/replace sets, disjoint source-absent additions, and exact overlay coverage. Current counts are four omissions, 15 replacements, one Task 5 addition, six Task 6 additions, and ten overlays; computed Task 5/final path totals remain 1,902/1,908.
...
const nonOverlayReplacements = new Set(["AGENTS.md", "README.md", "docs/project-log.md", "docs/project-state.md", "package.json"]);
assert.deepEqual([...nonOverlayReplacements].sort(), manifest.replacePaths.filter(path => !manifest.overlayPaths.includes(path)).sort(), "Exact non-overlay replacement coverage");
const expectedOverlays = manifest.replacePaths.filter(path => !nonOverlayReplacements.has(path)).sort();
...
assert.deepEqual(fields.map(field => manifest[field].length), [4, 15, 1, 6, 10]);
assert.equal(source.size, 1905);
...
console.log("PASS: JSON equality; normalized sorted unique arrays; source/add/replace/overlay contracts; counts omit=4 replace=15 task5Add=1 task6Add=6 overlay=10; source=1905 Task5=1902 final=1908");
'@
```
...
Task 4A full source QA exposed baseline blockers in the ordinary Browser-QA environment, authenticated fixture isolation, connected Admin-action rendering, carousel synchronization, homepage measurement readiness, and affected acceptance contracts. PR #206 merged all 43 reviewed Task 4A paths plus the separate `tests/family-friends-server-workload.test.mjs` prepublication repair into exact source `e74045c2fc85c2cb4df176fdb1aff2137c4d9848`. Task 5 exports those source blobs directly; they are no longer overlays. These changes alter no provider configuration, schema, migration, or rebrand behavior.
...
- Modify: `AGENTS.md`
- Carry only the ten manifest-declared relock documents from `codex/atmoshaper-phase2-relock`; parity/runtime/test/config/snapshot files now come from the locked source
...
Run this gate in one PowerShell session **before inspecting, creating, or writing the destination**. It requires the exact relock branch, a clean tracked/untracked state, a manifest read from the branch's committed `HEAD`, no working-manifest delta, manifest source agreement with tracked and live `main`, and relock ancestry. Every external command has an explicit exit check. Do not use the local `main` branch pointer as export authority; it may intentionally remain at the historical pre-merge parent.

```powershell
$sourceRepo = (Resolve-Path -LiteralPath "C:\Users\derri\code\my_projects\massagelab").Path
$overlayBranch = "codex/atmoshaper-phase2-relock"
$manifestRepoPath = "docs/rebrand/atmoshaper-export-manifest.json"
$manifestPath = Join-Path $sourceRepo $manifestRepoPath

$currentBranch = (& git -C $sourceRepo branch --show-current).Trim()
if ($LASTEXITCODE -ne 0) { throw "Could not read the relock branch." }
if ($currentBranch -cne $overlayBranch) { throw "Task 5 requires the reviewed relock branch." }

$statusLines = @(& git -C $sourceRepo status --porcelain=v1 --untracked-files=all)
if ($LASTEXITCODE -ne 0) { throw "Could not verify relock worktree status." }
if ($statusLines.Count -ne 0) { throw "Relock branch must be committed and clean before Task 5." }

$relockHead = (& git -C $sourceRepo rev-parse HEAD).Trim()
if ($LASTEXITCODE -ne 0 -or $relockHead -notmatch '^[a-f0-9]{40}$') { throw "Invalid relock HEAD." }
$committedManifestLines = @(& git -C $sourceRepo show "${relockHead}:$manifestRepoPath")
if ($LASTEXITCODE -ne 0 -or $committedManifestLines.Count -eq 0) { throw "Could not read the committed export manifest." }
try { $manifest = ($committedManifestLines -join "`n") | ConvertFrom-Json } catch { throw "Committed export manifest is invalid JSON." }
$sourceSha = [string]$manifest.sourceCommit
if ($manifest.schemaVersion -ne 1 -or $sourceSha -notmatch '^[a-f0-9]{40}$') { throw "Committed export manifest has an invalid source contract." }

& git -C $sourceRepo diff --quiet HEAD -- $manifestRepoPath
$manifestDiffExit = $LASTEXITCODE
if ($manifestDiffExit -eq 1) { throw "Working export manifest differs from committed HEAD." }
if ($manifestDiffExit -ne 0) { throw "Could not compare the working and committed export manifests." }

$trackedMain = (& git -C $sourceRepo rev-parse origin/main).Trim()
if ($LASTEXITCODE -ne 0 -or $trackedMain -notmatch '^[a-f0-9]{40}$') { throw "Could not resolve tracked main." }
$liveMain = (& gh api repos/dsbowersock/massagelab/git/ref/heads/main --jq .object.sha).Trim()
if ($LASTEXITCODE -ne 0 -or $liveMain -notmatch '^[a-f0-9]{40}$') { throw "Could not resolve live GitHub main." }
if ($sourceSha -cne $trackedMain -or $sourceSha -cne $liveMain) { throw "Manifest, tracked main, and live main disagree." }

& git -C $sourceRepo merge-base --is-ancestor $sourceSha $relockHead
$ancestryExit = $LASTEXITCODE
if ($ancestryExit -eq 1) { throw "Relock branch does not descend from the manifest source." }
if ($ancestryExit -ne 0) { throw "Could not verify relock ancestry." }
```

Keep `$sourceRepo`, `$overlayBranch`, `$manifest`, `$sourceSha`, and `$relockHead` in this unchanged session through Step 5. Any failure stops Task 5 before destination creation and returns to source selection/review.
...
Continue in the same Step 1 PowerShell session. Keep `$tempRoot`, `$manifest`, the locked Git values, and resolved paths through Step 5. Remove only the validated task-specific `$tempRoot` after the overlay archive is extracted.
...
Archive only the manifest's exact overlay paths from the reviewed relock branch, then extract them over the destination. The merged source already owns all reviewed runtime, test, config, and snapshot bytes, including the migration-parity package script; none of those paths may be overlaid again.
...
foreach ($path in $manifest.overlayPaths) {
  $sourceBlob = (& git -C $sourceRepo rev-parse "${sourceSha}:$path").Trim()
  $overlayBlob = (& git -C $sourceRepo rev-parse "${overlayBranch}:$path").Trim()
  if ($LASTEXITCODE -ne 0 -or $sourceBlob -eq $overlayBlob) {
    throw "Overlay path does not intentionally differ from the locked source: $path"
  }
}
...
Read the complete `sourceSha..overlayBranch` diff for the ten overlay paths and stop if any branch-only file is absent from the manifest or any declared overlay is byte-identical to the source. Confirm `package.json` is not in that diff.
...
Also verify that every `replacePaths` and `overlayPaths` entry exists, every declared omit path is absent, and all manifest arrays are sorted and duplicate-free. The reviewed contract has four omissions, 15 replacements, one Task 5 addition, six Task 6 additions, and ten overlays. The staged Task 5 set has 1,902 paths and the final set after six Task 6 additions has 1,908. Compare changed content with the Markdown difference contract. Stop on any unexplained path, byte-identical overlay, or absent declared Task 5 path. Task 6 add paths remain intentionally absent until Task 6.
...
- Consumes: complete staged destination tree, source evidence, exact disposable-QA authorization, and provider-owned lifecycle operations.
- Produces: fresh initial commit on new-repo `main` plus a redacted receipt proving the one-cycle QA project was deleted and is absent.

**Why:** The initial root must be verified before publication and must state exact lineage.

**Impact/Compatibility:** Local new repository plus one explicitly authorized temporary Neon QA project that is created for this cycle and deleted in the outer cleanup boundary. Production and every prior QA project remain untouched.

**Verification:** Full destination gate passes; the fresh/empty/non-production project gate and exact repository migration status pass; synthetic-only parity snapshots match; fixture cleanup reaches zero; project deletion and post-delete absence pass; audits pass; commit parent list is empty; worktree is clean.
...
The [external-account checklist](../../rebrand/atmoshaper-external-account-checklist.md#disposable-database-parity-lifecycle) is the canonical provider-safety policy; this step is its sole operative sequence. Before any provider action, read the repository `neon-postgres` skill and current [official Neon MCP action documentation](https://neon.com/docs/ai/neon-mcp-server). Use the provider-owned MCP actions available at execution time (`create_project`, `describe_project`, `get_database_tables`, read-only `run_sql`, `delete_project`, and `list_projects`) or a reviewed current official equivalent. Do not guess CLI flags, use a production-connected MCP context, create a branch from production, restore/import data, or substitute an existing project.

The coordinator owns one outer lifecycle guard beginning before `create_project` and ending only after post-delete absence proof. Hold the ephemeral cycle label, project identifier, runtime/direct connection values, and approved fingerprint only in the current process/session; never print them, put them in command history, persist them to a file, or include them in Git/evidence. The redacted receipt records timestamps, environment role, action pass/fail, repository migration count/status, fixture zero counts, deletion pass/fail, and absence pass/fail only.

Execute this ordered lifecycle:

1. Generate a unique, non-user-derived cycle label in memory and call provider `create_project` for a **new project**, not a branch. Store the returned project lease only in memory. Immediately use `describe_project` to prove creation occurred in this cycle, the project is independent of the known production project, and no restore/import/production parent was used. If creation returns an uncertain result, reconcile by the exact ephemeral label with `list_projects`; zero or more than one match is a stop condition, and every uniquely identified cycle-owned project still enters the outer deletion boundary.
2. Obtain the new project's runtime/direct connection values without displaying them. Before either fingerprint mode, bind the cycle values to `MASSAGELAB_BROWSER_QA_DATABASE_URL`, `MASSAGELAB_BROWSER_QA_DIRECT_URL`, `DATABASE_URL`, and `DIRECT_URL`, and set `MASSAGELAB_BROWSER_QA_DATABASE=1` plus `VERCEL_ENV=preview`; the repository guard requires the QA pair and both safety flags even in print mode. Capture each silent npm invocation's stdout in memory, save and validate `$LASTEXITCODE` immediately, and never render or persist either captured output. Require the print result to be exactly one 64-character lowercase hexadecimal line, bind it to `MASSAGELAB_BROWSER_QA_DATABASE_FINGERPRINT`, and require the expected-mode output to be exactly that one value before the first database query or migration. Also compare provider project identity with the separately read production identity in memory; equality stops the run. Do not record either identity or fingerprint.
3. After the fingerprint gate, call `get_database_tables` (and only a read-only catalog query if needed) against the cycle project. Require zero application tables and no `_prisma_migrations` table before deployment. Default provider system objects are not application data. Any unexpected user relation, prior migration table, or non-empty evidence stops the run and proceeds directly to outer cleanup.
4. Apply only the destination repository's committed migrations with `npm run prisma:migrate:deploy`, check its exit, then run `npx prisma migrate status` and check its exit. Both commands can include target details in ordinary output, so capture their complete stdout and stderr in memory, never stream or persist either, and expose only the generic step name, numeric exit code on failure, or sanitized pass/count evidence. Compare the exact sorted successful `_prisma_migrations.migration_name` values from a read-only provider query with the exact sorted directory names under `prisma/migrations`; require equality and record only the count/pass result. No ad hoc DDL, seed, production copy, or provider-side migration helper may replace the repository migration set.
5. Run all four `PLAYWRIGHT_CI_LANE` values exactly as in Task 4, then run migration parity with `ATMOSHAPER_MIGRATION_PARITY=1`. Do not update snapshots. The parity spec's fingerprint-gated, deterministic `.example.test` fixture helper is the only row writer; all other provider writes remain disabled, mocked, or read-only.
6. In an inner `finally`, allow the repository-owned per-project fixture cleanup to finish even after test failure, then use read-only aggregate queries to require zero matching fixture rows in every table the fixture owner can touch. Record table names and zero counts only. A nonzero count or unavailable query fails parity but never skips project deletion.
7. In the **outer `finally`**, call provider `delete_project` for every cycle-owned project identified by the lease/reconciliation step, regardless of setup, migration, test, fixture-cleanup, or receipt failure. Then call `list_projects` and, if addressable, `describe_project`; require the exact cycle-owned project and label to be absent. Clear all connection/fingerprint environment values and every in-memory captured-output variable in a nested `finally`. A deletion error, surviving project, ambiguous create result, or missing absence proof fails Task 7 and prohibits the initial commit.

Execute the fingerprint portion inside the outer lifecycle `try`, where `$cycleRuntimeUrl` and `$cycleDirectUrl` are the in-memory values returned for the newly created project:

```powershell
$env:MASSAGELAB_BROWSER_QA_DATABASE_URL = $cycleRuntimeUrl
$env:MASSAGELAB_BROWSER_QA_DIRECT_URL = $cycleDirectUrl
$env:DATABASE_URL = $cycleRuntimeUrl
$env:DIRECT_URL = $cycleDirectUrl
$env:MASSAGELAB_BROWSER_QA_DATABASE = "1"
$env:VERCEL_ENV = "preview"

$fingerprintOutput = @(& npm.cmd run --silent browser-qa:db:target -- --print-fingerprint)
$fingerprintExit = $LASTEXITCODE
if ($fingerprintExit -ne 0) { throw "Browser-QA fingerprint calculation failed." }
if ($fingerprintOutput.Count -ne 1) { throw "Browser-QA fingerprint calculation returned unexpected output." }
$cycleFingerprint = ([string]$fingerprintOutput[0]).Trim()
if ($cycleFingerprint -notmatch '^[a-f0-9]{64}$') { throw "Browser-QA fingerprint has an invalid format." }
$env:MASSAGELAB_BROWSER_QA_DATABASE_FINGERPRINT = $cycleFingerprint

$expectedOutput = @(& npm.cmd run --silent browser-qa:db:target -- "--expected-fingerprint=$cycleFingerprint")
$expectedExit = $LASTEXITCODE
if ($expectedExit -ne 0) { throw "Browser-QA expected-fingerprint verification failed." }
if ($expectedOutput.Count -ne 1 -or ([string]$expectedOutput[0]).Trim() -cne $cycleFingerprint) {
  throw "Browser-QA expected-fingerprint verification returned unexpected output."
}
# Never echo these values; clear them in the outer nested finally.
```

The required control flow is structural, not optional prose:

```text
cycleProjects = []
try {
  create/reconcile exactly one fresh project; add every cycle-owned match to cycleProjects
  prove provider identity + approved fingerprint + empty database
  apply repository migrations; prove exact migration-name equality
  try { run lanes and parity } finally { finish exact fixture cleanup; prove zero rows }
} finally {
  for each cycle-owned project: delete_project
  prove every cycle-owned project absent
  clear all ephemeral environment values and captured-output variables
}
```

Run the repository portion from the destination checkout in a fresh child inside that outer provider lifecycle, applying the same explicit-empty telemetry preflight and building that checkout immediately before the owned-server comparison. Because redirected `ProcessStartInfo` execution does not reliably resolve Windows command shims, the helper resolves every requested executable with `Get-Command -CommandType Application`, requires its absolute application path, and passes only that path to `ProcessStartInfo`:

```powershell
& pwsh.exe -NoProfile -Command @'
function Invoke-CapturedProcess {
  param([Parameter(Mandatory)][string]$FileName, [Parameter(Mandatory)][string[]]$ArgumentList)
  $application = Get-Command -Name $FileName -CommandType Application -ErrorAction SilentlyContinue | Select-Object -First 1
  if (-not $application -or -not $application.Path -or -not [IO.Path]::IsPathFullyQualified($application.Path)) {
    throw "Captured process application could not be resolved; raw diagnostics withheld."
  }
  $startInfo = [Diagnostics.ProcessStartInfo]::new()
  $startInfo.FileName = $application.Path
  ...
  try {
    ...
    throw "Captured process could not start or complete; raw diagnostics withheld."
  } finally {
    $process.Dispose()
  }
}
...
try {
  ...
  $telemetryResult = Invoke-CapturedProcess node @("--input-type=module", "-e", "import { assertMigrationParityTelemetryEnvironment } from './playwright.config.ts'; assertMigrationParityTelemetryEnvironment(process.env)")
  if ($telemetryResult.ExitCode -ne 0) { throw "Telemetry preflight failed (exit $($telemetryResult.ExitCode)); raw diagnostics withheld." }

  $expectedResult = Invoke-CapturedProcess npm.cmd @("run", "--silent", "browser-qa:db:target", "--", "--expected-fingerprint=$env:MASSAGELAB_BROWSER_QA_DATABASE_FINGERPRINT")
  if ($expectedResult.ExitCode -ne 0) { throw "Expected-fingerprint gate failed (exit $($expectedResult.ExitCode)); raw diagnostics withheld." }
  $expectedLines = @($expectedResult.Stdout -split '\r?\n' | Where-Object { $_ -ne "" })
  if ($expectedLines.Count -ne 1 -or $expectedLines[0].Trim() -cne $env:MASSAGELAB_BROWSER_QA_DATABASE_FINGERPRINT) { throw "Expected-fingerprint gate returned unexpected captured output." }

  $deployResult = Invoke-CapturedProcess npm.cmd @("run", "--silent", "prisma:migrate:deploy")
  if ($deployResult.ExitCode -ne 0) { throw "Repository migration deploy failed (exit $($deployResult.ExitCode)); raw diagnostics withheld." }

  $statusResult = Invoke-CapturedProcess npx.cmd @("prisma", "migrate", "status")
  if ($statusResult.ExitCode -ne 0) { throw "Repository migration status failed (exit $($statusResult.ExitCode)); raw diagnostics withheld." }
  Write-Output "Repository migration deploy/status: PASS; exact migration count must come from the redacted name-equality gate."

  $buildResult = Invoke-CapturedProcess npm.cmd @("run", "--silent", "build:browser-qa")
  if ($buildResult.ExitCode -ne 0) { throw "Browser-QA build failed (exit $($buildResult.ExitCode)); raw diagnostics withheld." }

  $parityResult = Invoke-CapturedProcess npm.cmd @("run", "--silent", "test:browser:migration-parity")
  if ($parityResult.ExitCode -ne 0) { throw "Migration parity failed (exit $($parityResult.ExitCode)); raw diagnostics withheld." }
  $paritySummary = [regex]::Match(($parityResult.Stdout + "`n" + $parityResult.Stderr), '(?m)^\s*(\d+)\s+passed\b')
  if (-not $paritySummary.Success -or [int]$paritySummary.Groups[1].Value -ne 22) { throw "Migration parity returned an unexpected captured pass count." }
  Write-Output "Browser-QA build: PASS; migration parity: PASS (22 tests); raw output withheld."
} finally {
  $telemetryResult = $expectedResult = $expectedLines = $deployResult = $statusResult = $buildResult = $parityResult = $paritySummary = $null
  Remove-Item Env:DATABASE_URL, Env:DIRECT_URL, Env:MASSAGELAB_BROWSER_QA_DATABASE, Env:MASSAGELAB_BROWSER_QA_DATABASE_URL, Env:MASSAGELAB_BROWSER_QA_DIRECT_URL, Env:MASSAGELAB_BROWSER_QA_DATABASE_FINGERPRINT -ErrorAction SilentlyContinue
}
'@
```

Expected: the provider lifecycle receipt passes every create/identity/fingerprint/empty/migration/fixture-zero/delete/absence gate; lane counts match source; all 22 migration tests pass; and all 24 screenshots compare. Any snapshot update, retained QA project, retained fixture, or unredacted identifier invalidates Task 7.
