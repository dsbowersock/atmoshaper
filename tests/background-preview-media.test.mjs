...
describe("background preview media", () => {
  ...
  it("keeps the carousel poster-first and sends play intent to every non-shell card", () => {
    ...
    assert.match(carouselSource, /const \[previewPreferenceEnabled, setPreviewPreferenceEnabled\] = useState\(true\)/)
    assert.match(carouselSource, /const previewPlaybackActive =\s*preferenceHydrated && previewPreferenceEnabled && active && !reducedMotion/)
