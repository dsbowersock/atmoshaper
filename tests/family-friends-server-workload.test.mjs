...
const PROJECT_STATE_VERIFIED_DATE_UPPER_BOUND = "2026-09-10"
...
describe("family-and-friends server workload baseline", () => {
  ...
  it("keeps stable launch-cost evidence aligned and current state dated", () => {
    for (const stableOperationalSource of [normalizedDeploymentSource, normalizedReleaseChecklistSource]) {
      assert.match(stableOperationalSource, /ordinary non-practice shell/i)
      assert.match(stableOperationalSource, /four logical ORM operations/i)
      assert.match(stableOperationalSource, /zero client bootstrap endpoints/i)
      assert.match(stableOperationalSource, /zero ordinary commerce snapshots/i)
      assert.match(stableOperationalSource, /public display catalog only/i)
      assert.match(
        stableOperationalSource,
        /(?:local timing `first`.{0,80}\bnot\b.{0,80}platform cold evidence|platform cold evidence.{0,80}\bnot\b.{0,80}local timing `first`)/i,
      )
    }
    assert.match(normalizedReleaseChecklistSource, /Keep live Stripe.{0,300}recorded as `NOT RUN`/i)
    const verifiedDateMatch = /^Verified: (\d{4}-\d{2}-\d{2})\r?$/m.exec(projectStateSource)
    assert.ok(normalizedProjectStateSource.startsWith("# AtmoShaper Project State ") && verifiedDateMatch, "project state must retain the fresh AtmoShaper identity and include one ISO-formatted Verified date")
  ...
  it("keeps the fresh-history boundary and stable migration evidence explicit", () => {
    const deploymentMigrationEvidence = namedFunctionSlice(
      normalizedDeploymentSource,
      "## Identity, Membership Schema, And Writer Rollout",
      "## SEO And Indexing",
    )
    const releaseMigrationEvidence = namedFunctionSlice(
    ...
    const completedMigrationOrder =
      /20260828120000_identity_method_safety.{0,160}20260828121000_identity_normalized_email_index.{0,160}20260828130000_membership_subscription_convergence.{0,160}20260901100000_auth_method_intent_two_factor_purposes.{0,160}20260901101000_auth_method_intent_registration_callback/i

    assert.match(
      projectLogSource,
      /\[source-locked MassageLab project log\]\(https:\/\/github\.com\/dsbowersock\/massagelab\/blob\/e74045c2fc85c2cb4df176fdb1aff2137c4d9848\/docs\/project-log\.md\)/,
    )
    assert.match(normalizedProjectLogSource, /rather than copying that history here/i)
    assert.doesNotMatch(projectLogSource, /^## 2026-0[1-8]-/m)
    assert.doesNotMatch(projectLogSource, /^## 2026-09-0[1-7]\b/m)

    for (const stableMigrationSource of [deploymentMigrationEvidence, releaseMigrationEvidence]) {
      assert.match(stableMigrationSource, completedMigrationOrder)
    }
    assert.match(
      releaseMigrationEvidence,
      /20260831120000_operational_rate_limit_bucket.{0,120}subsequently applied.{0,80}current/i,
    )
    assert.match(releaseCostControls, HISTORICAL_BROWSER_QA_RECEIPT_PATTERN)
    assert.match(releaseCostControls, /those counts are evidence for that candidate, not fixed totals for a future candidate/i)
  })
